...
The template is highly configurable and lets you enter data once and reuse it. Many macros have been defined. By setting values via these macros, you can set them once and reuse them in many places, reducing the work of updating them while ensuring consistency. Another reason for many of the macros is to collect the information that is needed to report the thesis, \ie to report the title(s), abstracts, keywords, and thesis in \gls{DiVA}, create an errata document (if needed), and, after the defense, to report the title(s) and grade in \gls{LADOK}.
...
\item [\texttt{\textbackslash authorsLastname\{\}}] the last name of the author\footnote{Note that the author's name can include a suffix such as \enquote{, Jr.} or \enquote{ Jr.}, \ie as the author prefers to write their name.}
...
\iflabelexists{tab:examplesOfNationalSubjectCategories}{Some examples of these codes are shown in \Cref{tab:examplesOfNationalSubjectCategories}.}
...
As described in \Cref{sec:cycle3BackgroundOfThesisAndDefense}, the errata have to be announced before the student starts their actual oral defense. I have not found a standard format for this, as a printed document. Looking at a number of errata that are in \gls{DiVA} for theses at \gls{KTH}, most have a title of \enquote{Errata} or \enquote{Errata for doctoral/licentiate thesis} followed by a reference to the thesis. This reference can be as short as the title of the thesis \& the author or as long as a full reference (with ISBN). The key is that it should be possible to identify the relevant thesis from the information at the top of the errata. \textbf{NB} If you use the \texttt{errata.tex} file in this project, it will automatically include this information.
