...
\begin{itemize}
    ...
    \item \textbf{Automated \gls{DOI} Discovery:} Entries lacking a \gls{DOI} or \gls{ISBN} are cross-referenced against Crossref's bibliographic search \gls{API}. If a high-confidence match ($>0.75$ similarity) is identified, the script emits an actionable suggestion for the author to add the canonical identifier to \fname{references.bib}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Direct arXiv / eprint Resolution:} While some arXiv preprints assign a standard \gls{DOI} (\eg via the \texttt{10.48550} prefix), many citations only supply an \texttt{eprint} field without an explicit \gls{DOI}. A dedicated lookup against the arXiv REST \gls{API} is needed to validate preprint identifiers and check whether a formal journal version has since been published.
