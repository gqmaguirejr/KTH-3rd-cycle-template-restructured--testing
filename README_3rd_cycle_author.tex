...
This document describes the \LaTeX{} thesis template that I have developed for use at \gls{KTH} for third-cycle theses. It is important to note that the template is \textbf{not prescriptive}, as not every thesis will have all of the parts that the template shows. For example, some theses will be monographs, while others will be compilation theses.
...
One of the hardest problems an author faces is getting started writing, \ie the blank sheet of paper -- empty file barrier. The template (located in the file \texttt{examplethesis.tex}) provides a \mbox{non-blank} starting point, helping avoid the blank-page barrier. Additionally, the template provides some initial structure, basically, an \gls{IMRAD} structure, so that there are hints of where to place material. Moreover, there are places (and notes) about material that the author should consider adding; for example, the \enquote{required reflections} section in the final chapter.
...
If you are using Overleaf and have opened the template from a \gls{URL}, click \textbf{File} in the upper left-hand corner. Then select \textbf{Make a copy}. Use a helpful project name,
I suggest you include your name in the project name so that when you share it with your supervisor(s), they will know it is your project. If you are using GitHub, you can clone the project from \url{https://github.com/gqmaguirejr/KTH-3rd-cycle-template-restructured}. You can also set up Overleaf to synchronize with your GitHub repository (using the \enquote{Integrations} $\rightarrow$ GitHub).

The second step will be to make sure that the information about you, your supervisor(s), and the degree is correct in the file \texttt{custom\_configuration.tex} - this information uses various macros as described in \Cref{sec:configuringCustomConfig}.
...
The template is highly configurable and lets you enter data once and reuse it multiple times. Many macros have been defined. By setting values via these various macros, you can set the value once and then use this value in many places, thus reducing the work to change them, while at the same time ensuring consistency. Another reason for many of the macros is to collect the information that is needed to report the thesis, \ie to report the title(s), abstracts, keywords, and thesis in \gls{DiVA}, create an errata document (if needed), and, after the defense, to report the title(s) and grade in \gls{LADOK}.
...
For the author, there are a number of macros defined to store information about the author, so that it can later be used in multiple places -- for example, the \gls{KTH} cover (produced with \texttt{\textbackslash kthcover)}, the title page (produced with \texttt{\textbackslash titlepage}, the \enquote{For DIVA} section\footnote{Note that the actual section name has \gls{DiVA} set in all caps - which hopefully should not occur in the thesis! If the string \gls{DiVA} set in all caps, does have to appear, then the section heading should be preceded by four euro signs and followed by four more euro signs (as is done in this document).} at the end of the thesis (produced with \linebreak[4]
{\small \texttt{\textbackslash divainfo\{pg:lastPageofPreface\}\{pg:lastPageofMainmatter\}}}), \linebreak[4]and possibly a \gls{JSON} file named \texttt{fordiva.json} produced as a byproduct of the \texttt{\textbackslash divainfo}. Additionally, the author's name will be included in the \gls{DiVA} entry, in the \gls{LADOK} grade entry, and on the diploma.

The author-related macros are:
\begin{description}[leftmargin=!, labelwidth =\widthof{\texttt{\textbackslash authorsFirstname\{\}}}]
\item [\texttt{\textbackslash authorsLastname\{\}}] the last name of the author\footnote{Note that the author's name can include a suffix such as \enquote{, Jr.} or \enquote{ Jr.}, \ie the suffix can be separated with a comma or not --- as the author prefers to write their name.}
...
\item [\texttt{\textbackslash authorsSchool\{\}}] the argument is generally of the form:\linebreak[4] \texttt{\textbackslash schoolAcronym\{XXX\}}. The currently supported school acronyms are: \enquote{ABE}, \enquote{CBH}, \enquote{EECS}, \enquote{ITM}, and \enquote{SCI}. These are defined in the file\linebreak[4] \texttt{school\_subjects\_3rdcycle.ins}. There is also a fake school XXX for use in the template.
\end{description}
...
\Needspace*{8\baselineskip}
If the student was working at a company, such as \enquote{Företaget AB}, or an organization, such as \enquote{CERN}, they can specify this using one of the following macros.
\begin{lstlisting}[style=latexExampleForAuthors]
...
\begin{lstlisting}[language={bash}, caption={Generating \LaTeX~commands for a \gls{KTH} supervisor B}, label=lst:supervisorB, basicstyle=\footnotesize]
...
Some additional configuration that the author might do is to set the values of the macros related to the course cycle, date of the thesis, degree/exam name, subject area, and if the work for the degree is done external to \gls{KTH} to set the host information (see the file \texttt{custom\_configuration.tex}). Consider the snippet below for a doctoral student in Information and Communication Technology (INFKTEKN):
\begin{lstlisting}[style=latexExampleForAuthors]
...
While these codes refer to research areas, these codes are also used at \gls{KTH} to indicate the area of the thesis. The guidance that I received from the Linköping University library was that one should try to use 5-digit codes when possible. 
...
The UN's \glspl{SDG} codes are for a field in the \gls{DiVA} record.
...
The thesis (if a monograph) or the kappa (if a compilation thesis) becomes a public document from the time of the posting (\enquote{nailing up}\footnote{\foreignlanguage{swedish}{In Swedish: spikningen}}) of the thesis in public. At \gls{KTH}, the thesis should \enquote{be made available (posted) in both printed and electronic form, at least three weeks prior to the doctoral defense or licentiate seminar} (see \href{https://www.kth.se/en/biblioteket/publicera-analysera/hantera-publikationer/om-att-spika-avhandling-1.854781}{About posting a thesis}).

The printer must send printed copies of the full thesis to the National Library of Sweden and a list of repository libraries distributed around Sweden. Additionally, it is customary to send the thesis to others, such as the department(s) in other universities that might be interested in the thesis. At \gls{KTH}, the public document is made available via \gls{DiVA} and a printed copy is always available on request to the registrar.
...
A compilation thesis, in Swedish \textit{\foreignlanguage{swedish}{sammanläggning}}, consists of a \enquote{Kappa}, \ie a comprehensive summary of the thesis, and a set of publications (included in the thesis\footnote{Note that the KI documentation refers to these publications as \enquote{the constituent papers}.}). Ideally, the Kappa helps establish the so-called \enquote{red thread} that ties the set of included publications together. As the kappa is written \textit{after} the publications that are included in the thesis, it can exploit the information from all of the separate publications --- (ideally) to tie them all together in a readily understandable way.
...
\label{sec:cycle3BibMgmtpackaage}

The template is designed to work with the \BibLaTeX{} package, as it is more modern and supports more types of entries; for example, datasets and software. Additionally, \BibLaTeX{} supports a number of \textit{Non-standard Types}\footnote{Such as: artwork, audio, commentary, image, jurisdiction, legislation, legal, letter, movie, music, performance, review, standard, and video}, see §2.1.3 of the documentation for the \BibLaTeX{} package, \ie \textit{The \BibLaTeX{} Package: Programmable Bibliographies and Citations}. \BibLaTeX{} also offers additional citation functionality, such as \texttt{\textbackslash fullcite}. \BibLaTeX{} supports a large number of languages (see \enquote{Table 2: Supported Languages} of the package documentation). See also \href{https://www.overleaf.com/learn/latex/Bibliography_management_with_biblatex}{Bibliography management with biblatex}.
...
The template \textbf{only} works with \LuaLaTeX.  Overleaf makes many of the fonts you might use available. If you are running locally, you must ensure you have installed the fonts you need.

If you are using Overleaf, it is easy to select the compiler (\ie \TeX{} engine) by using the drop-down menu \enquote{File} $\rightarrow$ \enquote{Settings} $\rightarrow$ \enquote{Compiler}.
...
I suggest avoiding defined acronyms in abstracts; instead, spell them out rather than using the glossary commands. This is because the \texttt{glossaries} package (used to support acronyms) does not directly support multiple languages, and I do not understand how to programmatically create plurals of acronyms in Swedish or other languages. Even in an English abstract, it is desirable to avoid using the glossary commands, as this makes subsequent processing of the abstracts harder, since one has to make sure that the list of acronyms and their definitions are provided to any program that will process this \LaTeX{}  source code. For this reason, later versions of this template attach the \texttt{acronyms.tex} file.
...
You will likely find parts of the template you do not want/need. One way to deal with this is to delete them; another is to comment them out. Personally, I like to comment things out in case I actually want to read them in the \LaTeX{} file or uncomment them later. To comment out a portion of the file, simply use the \texttt{comment} environment:
...
What can you do when the fonts you use are missing characters that you need to use? One solution is to use a font that includes the character(s) you want, then use them where you need them. For example, one can access additional symbols with code such as:
...
\newfontfamily\mystixmathfontSSa{STIX Two Math}[StylisticSet=1]
\end{lstlisting}
\noindent This defines a new font that utilizes the Stylistic Set 1 <ss01> - giving you access to roundhand glyphs rather than chancery
...
Inserting the \gls{PDF} of publications is very simple, as shown in \Cref{lst:InsertingPDFofIncludedPublication}. You can set the scaling of the included pages, specify the page range to include, add a frame around the page, and even include a running heading. The last probably requires enabling fancy headings for the pages. Also note that you need to include \texttt{\textbackslash usepackage\{pdfpages\}}\footnote{This is included by default in \texttt{lib/includes.tex}.}. If you want to include an image, you can use \texttt{\textbackslash includegraphics\{\textit{filename}\}} or perhaps scaled as \texttt{\textbackslash includegraphics[scale=0.90]\{\textit{filename}\}}.
\Needspace*{10\baselineskip}
...
For those who want to understand the details of how the code works, see the following subsections and the section on \enquote{Divider pages} in the \texttt{README\_for\_3rd\_cycle\_version.tex} file.
...
As described in \Cref{sec:cycle3BackgroundOfThesisAndDefense}, the errata have to be announced before the student starts their actual oral defense. I have not found a standard format for this, as a printed document. Looking at a number of errata that are in \gls{DiVA} for theses at \gls{KTH}, most have a title of \enquote{Errata} or `Errata for doctoral/licentiate thesis'' followed by a reference to the thesis. This reference can be as short as the title of the thesis \& the author or as long as a full reference (with ISBN). The key is that it should be possible to identify the relevant thesis from the information at the top of the errata. \textbf{NB} If you use the \texttt{errata.tex} file in this project, it will automatically include this information.
...
\textasciicircum\textasciicircum\textasciicircum\textasciicircum\textasciicircum \textasciicircum 01d792 - for a \enquote{MATHEMATICAL SANS-SERIF BOLD ITALIC CAPITAL GAMMA}. You should choose the \enquote{SANS-SERIF BOLD ITALIC} version of the character, since the title is set in Figtree Bold, a sans-serif bold font. In the \texttt{defines.tex} file you can find an include of a file of \enquote{Mathematical Alphanumeric Symbols, U+1D400 - U+1D7FF}  unicode characters that have been defined to render the character using the STIX Two Math font\footnote{the file's name is \fname{unicode\_blocks/U+1D400-U+1D7FF-Mathematical Alphanumeric Symbols}}.
...
\Needspace*{5\baselineskip}
\textbf{NB} You can insert Greek characters such as Γ, Δ, α, β, γ, \ldots\,, by directly entering them, but if you place these into a title, they will \textit{not} be set in boldface, since they are rendered using the STIX Two Math font which knows nothing about the boldface being used for the title (and, as noted earlier, this font actually does not have a bold version).
...
However, if we cut the title out from the property display and place it into an \gls{HTML} file, such that as shown in the list below. One gets the browser view as shown in \Cref{fig:titleInBrowser}.
