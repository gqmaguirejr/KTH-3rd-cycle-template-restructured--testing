...
\hypersetup{
 plainpages=false,
 colorlinks=true,       % if colored text is acceptable in print
                        % PDF related options
 pdfborder={0 0 0},          % Suppresses print bounding boxes
...
% Put the title, author, keyword, and other information into the PDF meta information
...
\hypersetup{
	colorlinks  = true,
	% breaklinks  = true, % Not needed with hyperref-generic driver
	linkcolor   = \linkscolor,
	urlcolor    = \urlscolor,
	citecolor   = \refscolor,
	% anchorcolor = black % Not needed with hyperref-generic driver
}
...
\makeatletter
% Restore the standard LaTeX2e definition of \@makefntext so the tagging parser succeeds:
...
\generalExpl{Keep in mind that most of your potential readers are only going to read your \texttt{title} and \texttt{abstract}. This is why the abstract must give them enough information to decide whether this document is relevant to them. Otherwise, they will likely ignore the rest of your document.}
...
An abstract is (typically) about 250 to 350 words (1/2 A4-page) with the following components:
...
\begin{itemize}
  ...
  \item List your keywords separated by commas (\enquote{,}).
\end{itemize}    
...
\warningExpl{Inside the following \texttt{scontents} environment, you cannot use a \textbackslash include{filename}, as the command itself, rather than the file's contents, will end up in the \enquote{For DiVA} information. Additionally, you should not use a straight double quote character in the abstracts or keywords, but rather use two single quote characters instead.}
...
\engExpl{The set of languages that can be easily included is based on those languages that were used in theses at KTH and those that should be available.}
...
% --- Start of a monograph or the body of the kappa ---
...
As shown in RFC 1235\,\cite{ioannidis_coherent_1991}, multicast is useful for xxxx. A number of different \glspl{OS} have been used in this work, such as the following \glspl{OS}: UNIX, Linux, Windows, \etc. The main focus is one \gls{OS}, namely Linux.
...
Det finns även ett syfte till som kan vara värt att beakta och det är att du som student skall ta examen och att du måste bevisa, i din avhandling, att du uppfyller examensmålen. %Dessa mål sammanfaller med kursmålen för examensarbetskursen. 
}
...
Tänk på att företaget ibland måste stå tillbaka i sin önskan och förväntan på projektets resultat till förmån för ny eller kompletterande ingenjörserfarenhet och vetenskap (din avhandling). Det är du som student som bestämmer och löser fördelningen mellan dessa två intressen men se till att alla är informerade. }
...
\generalExpl{Describe the boundary/limits of your thesis and what you are explicitly not going to do. This will help you bound your efforts, as you have clearly defined what is out of scope for this thesis. Explain the delimitations. These are all the things that could affect the study if they were examined and included in your thesis.}
...
\Cref{ch:background} presents relevant background information about xxx.  \Cref{ch:methods} presents the methodology and method(s) used to solve the problem. 
...
how prior work/prior research will be applied or used in your
...
\begin{table}[!ht]
  \begin{center}
    ...
    \begin{tabular}{l S[table-format=4.6]} % <-- Alignments: 1st column left, 2nd decimal aligned
    \toprule
...
Entangled states are an important part of quantum cryptography and are also relevant in other domains. This concept might be relevant for neutrinos; see, for example, \cite{kim_small-mass_2016}.
...
Carrier clouds have been proposed to reduce the delay between users and the cloud server that provides them with content. However, the question remains how to find available resources in such a carrier cloud. One approach has been to disseminate resource information using an extension to OSPF-TE, see Roozbeh, Sefidcon, and Maguire \cite{roozbeh_resource_2013}.
...
Give a theoretical description of the scientific or engineering methodology you are going to use and why you have chosen this method. What other methods did you consider, and why did you reject them?\\
...
\begin{table}[!ht]
  \begin{center}
    ...
    \begin{tabular}{l c} % <-- Alignments: 1st column left, 2nd centered
    \toprule
...
\sweExpl{Det här avsnittet är helt enkelt för att visa ett exempel på hur du kan inkludera kod i din avhandling - det här är inte ett avsnitt du skulle ha i din avhandling.}
...
\engExpl{This section is simply to show some examples of how you can draw your own figures for in your thesis --- this is not a section you would have in your thesis.}
\sweExpl{Det här avsnittet är helt enkelt för att visa ett exempel på hur du kan rita dina egna figurer i din avhandling --- det här är inte ett avsnitt du skulle ha i din avhandling.}

\Needspace*{5\baselineskip}
These figures are just some examples to show that you can draw your own figures for use in your thesis. This has two advantages: \first you do not have to worry about copyrights --- as these are your own figures, and \Second the text is now readable and not simply a picture of text --- so screen readers can read the figure's contents to someone who is listening to the contents of your thesis.
...
\begin{table}[!ht]
  \begin{center}
    ...
    \begin{tabular}{l S[table-format=4.2] S[table-format=3.2]} % <-- Alignments: 1st column left, 2nd and 3rd columns decimal aligned
    \toprule
...
\put(1439.0,131.0){\rule[-0.200pt]{0.400pt}{155.380pt}}
\end{picture}
\caption[A GNUplot figure]{Processing time vs. payload length}
...
\[
	\text{bit rate} = \frac{1}{\frac{\text{time}_{\text{byte}}}{8}} = \qty{20.03}{\kilo\bit\per\second}
\] 

\Cref{tab:majorMarkupLMDetailedResult} shows another table in which some values have been set in bold (using \textbackslash boldentry) to emphasize them. Note how the \texttt{S} formatting has been modified so that it considers the weight of the characters, enabling it to decimal-align even these bold-faced numbers with the numbers in the column above them.
...
\begin{table}[!ht]
    ...
    \begin{tabular}{l *{2}{S[detect-weight,mode=text,table-format=3.2]}}
        ...
        protein (g) & 97.2 & 100.0 \\
        salt (mg) & 9.7 & 9.3 \\
        \midrule
        \textbf{Average customer rating in \%} & \boldentry 82.2 & \boldentry 89.9 \\
        \bottomrule
    \end{tabular}
\end{table}
\FloatBarrier
