...
Note that each line of the annotation has a prefix of the form `<number>:' followed by a name, equal sign, and a quoted value. Internally, this information is encoded into \textit{parts} and stored by Biber into the `.bbl' file.
...
To render these annotations in print, one can define \BibLaTeX\ formatting hooks (as shown in \Cref{lst:biberHookExample1}). You can test for active annotations using \texttt{\textbackslash haspartannotation[} and retrieve values with \texttt{\textbackslash getpartannotation}. In this code the equal authorship in indicated with a superscript asterisk, while the name gets an  \texttt{\textbackslash orcidlink} hyperlink, \ie beside the name is the official green, circular \gls{ORCID} icon as an actionable, live hyperlink targeting the author's \gls{ORCID} record page. The icon itself is drawn as a resolution-independent vector path using TikZ/PDF literals rather than a raster image.

\begin{lstlisting}[style=latexExampleForAuthors, escapechar=|, basicstyle=\footnotesize, caption={{Example of a \LaTeX{} hook for equal contribution and ORCID icons}}, label=lst:biberHookExample1]
\usepackage{biblatex}
\usepackage{orcidlink}

% Render both the equal-contribution symbol and the ORCID icon next to the name
\renewcommand*{\mkbibnamefamily}[1]{%
  #1%
  % Check for equal contribution annotation
  \haspartannotation[author][default][\the\value{listcount}]{equal}{%
    \textsuperscript{*}%
  }{}%
  % Check for ORCID annotation
  \haspartannotation[author][default][\the\value{listcount}]{orcid}{%
    \,\orcidlink{\getpartannotation[author][default][\the\value{listcount}]{orcid}}%
  }{}%
}
\end{lstlisting}

As an example of this for \textbackslash fullcite\{doe2025collaborative\}:
\newbox\orcidbox
...
\begingroup
\directlua{
  function inspect_box0()
    local b = tex.box[0]
    if not b then
      tex.print("[box0 is nil]")
      return
    end
    if not b.list then
      tex.print("[box0.list is nil (empty box)]")
      return
    end
    ...
    local function walk(head)
      for n in node.traverse(head) do
        local ntype = tostring(node.type(n.id))
        table.insert(trace, ntype)
        if n.id == GLYPH then
          table.insert(chars, utf8.char(n.char))
        elseif n.id == HLIST or n.id == VLIST then
          if n.list then walk(n.list) end
        end
      end
    end
    ...
    local collected = table.concat(chars)
    local nodetypes = table.concat(trace, ", ")

    -- Print directly onto the typeset page in magenta
    tex.print(string.char(92) .. "textcolor{magenta}{[chars: " .. collected .. " | nodes: " .. nodetypes .. "]}")
    
    -- Also print to the compilation terminal/log
    texio.write_nl("--- BOX 0 INSPECTION START ---")
    texio.write_nl("Collected string: " .. collected)
    texio.write_nl("Node sequence:    " .. nodetypes)
    texio.write_nl("--- BOX 0 INSPECTION END ---")
  end
}
...
\renewcommand*{\mkbibnamefamily}[1]{%
  ...
  \haspartannotation[author][default][\curridx]{orcid}{%
    \setbox0=\hbox{\getpartannotation[author][default][\curridx]{orcid}}%
    \directlua{inspect_box0()}%
  }{}%
}
...
\begin{table}[!htp]
    ...
    \begin{tabular}{L{4.2cm} L{4.5cm} L{5.3cm}}
      \hline
...
@article{maguire2026pipeline,
  ...
  author+an = {
    1:orcid="0000-0002-6066-7469", 
      ...
      software="lead", 
      writing_original="lead";
    2:orcid="0000-0001-2345-6789", 
      supervision="lead", 
      writing_review="supporting"
  },
...
@article{maguire2026pipeline,
  author    = {Maguire, Jr., Gerald Q. and Smith, Jane},
  author+an = {
    1:orcid="0000-0002-6066-746X", credit="Conceptualization, Software, Writing - original draft";
    2:orcid="0000-0001-2345-6789", credit="Supervision, Writing - review & editing"
  },
  title     = {Automated Metadata Integrity in Academic Publishing},
  journal   = {Journal of Academic Stewardship},
  year      = {2026},
...
Which of these two is the best method for KTH's doctoral thesis students?
...
\newbibmacro*{author:credit:roles}{%
  \begingroup
  \def\do{%
    \hasitemannotation[\currentname][credit]{%
      \par\small\textbf{\namepartfamily}: \getitemannotation[\currentname][credit]%
    }{}%
  }%
...
This setup allows automated Python/CI validation pipelines to extract and cross-check contributor statements directly from \texttt{.bib} files without manual re-entry.
...
The following generates some example output when using Approach B for the CReDiT information in the.bib file:
\begingroup
...
\begin{itemize}
    \item \textbf{Biber `.bib` Annotation Delimiters:} Item annotations must be delimited by semicolons (`;`), not commas (`,`), with the item index explicit on each part (e.g., `1:conceptualization="lead"; 1:software="lead"`).
    \item \textbf{Kebab-Case Slugs:} Adopting the standard CReDiT simplified names (e.g., `writing-original-draft`, `data-curation`) avoids unescaped underscore collisions in TeX math mode and simplifies downstream JSON serialization.
...
To define the slug-to-text mappings:
\begin{lstlisting}[style=latexExampleForAuthors, escapechar=|, basicstyle=\footnotesize, caption={{Defining slug-to-text mappings}}, label=lst:biberCreditCommandSlugs]
% ==============================================================================
% CReDiT Role Slug-to-Text Mappings
% ==============================================================================
\usepackage{etoolbox}

% Define the formatted title string for each canonical CReDiT slug
\csdef{credit@conceptualization}{Conceptualization}
\csdef{credit@data_curation}{Data curation}
\csdef{credit@formal_analysis}{Formal analysis}
\csdef{credit@funding_acquisition}{Funding acquisition}
\csdef{credit@investigation}{Investigation}
\csdef{credit@methodology}{Methodology}
\csdef{credit@project_administration}{Project administration}
\csdef{credit@resources}{Resources}
\csdef{credit@software}{Software}
\csdef{credit@supervision}{Supervision}
\csdef{credit@validation}{Validation}
\csdef{credit@visualization}{Visualization}
\csdef{credit@writing_original}{Writing -- original draft}
\csdef{credit@writing_review}{Writing -- review \& editing}

% Lookup macro: formats the mapped string, or safely detokenizes if unknown
\newcommand{\formatcreditrole}[1]{%
  \ifcsdef{credit@#1}{\csuse{credit@#1}}{\detokenize{#1}}%
}
\end{lstlisting}
...
For Pattern A:
\begin{lstlisting}[style=latexExampleForAuthors, escapechar=|, basicstyle=\footnotesize, caption={{Define author:credit:roles macro}}, label=lst:biberCreditRolesMacro]
\newbibmacro*{author:credit:roles}{%
  \begingroup
  % Example for checking explicit role-as-key annotations (Pattern A)
  \newcommand{\checkrole}[1]{%
    \hasitemannotation[\currentname][##1]{%
      \par\small\hspace*{1em}\textbf{\formatcreditrole{##1}}%
      \space(\getitemannotation[\currentname][##1])%
    }{}%
  }%
  % Iterate over canonical slugs
  \checkrole{conceptualization}%
  \checkrole{software}%
  \checkrole{writing_original}%
  \checkrole{writing_review}%
  \endgroup
}
\end{lstlisting}
...
Key Built-in Macros.
\begin{itemize}
    \item \textbackslash hasitemannotation[<item-index>][<annotation-name>]: Checks whether the name at <item-index> possesses the specified annotation slug.

    \item \textbackslash getitemannotation[<item-index>][<annotation-name>]: Retrieves the assigned value (\eg lead, equal, supporting, or true) for that slug.
\end{itemize}

Implementation: Iterating Through the Slugs for Each Author
Using etoolbox list iteration (\texttt{\textbackslash docsvlist)}, you can step through each author and test each of the 14 standard CReDiT slugs systematically:
...
% 1. List of all 14 standard CReDiT slugs to test
\newcommand*{\CreditSlugList}{%
  conceptualization,%
  data_curation,%
  formal_analysis,%
...
% 2. Helper macro to check and print an individual role for the current author index
\newcommand*{\PrintCreditRoleForCurrentAuthor}[1]{%
  \hasitemannotation[\currentname][#1]{%
    \par\hspace*{2em}%
    \formatcreditrole{#1}: \textit{\getitemannotation[\currentname][#1]}%
  }{}%
}

% 3. Format driver for each author in the author list
\DeclareNameFormat{creditRolesPerAuthor}{%
  % Print the author's family name (and initials if desired)
  \par\small\textbf{\namepartfamily}%
  % Iterate through all 14 slugs for \currentname (the active author index)
  \renewcommand*{\do}[1]{\PrintCreditRoleForCurrentAuthor{##1}}%
  \expandafter\docsvlist\expandafter{\CreditSlugList}%
}

% 4. Bibliography macro to trigger the iteration over the entry's authors
\newbibmacro*{author:credit:roles}{%
  \begingroup
  \indexnames[creditRolesPerAuthor]{author}%
  \endgroup
}
\end{lstlisting}
...
To retrieve or manipulate the current citation entry key, \BibLaTeX\ provides the following internal macros and mechanisms:
...
\textbackslash thefield\{entrykey\}: Prints or retrieves the active .bib entry key (e.g., maguire2026pipeline).
