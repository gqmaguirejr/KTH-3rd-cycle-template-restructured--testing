...
Biber's data annotation syntax enables bibliography entries to carry rich, structured metadata attached directly to individual author or field items without modifying the underlying \BibLaTeX\ data model. For example:
...
@article{doe2025collaborative,
  ...
  author+an = {
    1:orcid="0000-0002-6066-7469", equal=true;
    2:orcid="0000-0001-2345-6789", equal=true;
...
\begin{enumerate}
    \item Semicolons (\texttt{;}) separate distinct list items (e.g., Author~1, Author~2, Author~3).
    \item Commas (\texttt{,}) separate multiple annotations declared on the same item (\eg \texttt{orcid=...} and \texttt{equal=true}).
\end{enumerate}

To render these annotations in print, define \BibLaTeX\ formatting hooks. In the preamble, test for active annotations using \texttt{\textbackslash hasitemannotation} and retrieve values with \texttt{\textbackslash getitemannotation}:
...
\renewcommand*{\mkbibnamefamily}[1]{%
  ...
  \hasitemannotation[\currentname][equal]{%
    \textsuperscript{*}%
  }{}%
  % Check for ORCID annotation
  \hasitemannotation[\currentname][orcid]{%
    \,\orcidlink{\getitemannotation[\currentname][orcid]}%
  }{}%
}
\end{lstlisting}
    

Currently, there are no hardcoded built-in keyword annotations reserved specifically for \gls{CReDiT} (Contributor Roles Taxonomy, ANSI/NISO Z39.104-2022). However, standard canonical identifiers can be utilized directly as annotation keys, as outlined in \cref{tab:CReDiT_annotation}.

\begin{table}[!htp]
    \centering
    \caption{CReDiT roles for author data annotations in \BibLaTeX}
    \label{tab:CReDiT_annotation}
    \begin{tabular}{L{4.2cm} L{4.5cm} L{5.3cm}}
      \hline
      \textbf{CReDiT Role} & \textbf{Canonical Key} & \textbf{Typical Degrees of Contribution} \\
      \hline
      Conceptualization          & \texttt{conceptualization}     & lead, supporting, equal (or true) \\
      Data curation              & \texttt{data\_curation}         & lead, supporting, equal \\
      Formal analysis            & \texttt{formal\_analysis}       & lead, supporting, equal \\
      Funding acquisition        & \texttt{funding\_acquisition}   & lead, supporting, equal \\
      Investigation              & \texttt{investigation}         & lead, supporting, equal \\
      Methodology                & \texttt{methodology}           & lead, supporting, equal \\
      Project administration     & \texttt{project\_administration}& lead, supporting, equal \\
      Resources                  & \texttt{resources}             & lead, supporting, equal \\
      Software                   & \texttt{software}              & lead, supporting, equal \\
      Supervision                & \texttt{supervision}           & lead, supporting, equal \\
      Validation                 & \texttt{validation}            & lead, supporting, equal \\
      Visualization              & \texttt{visualization}         & lead, supporting, equal \\
      Writing -- original draft  & \texttt{writing\_original}     & lead, supporting, equal \\
      Writing -- review \& editing & \texttt{writing\_review}     & lead, supporting, equal \\
      \hline
    \end{tabular}
\end{table}

There are two primary paradigms for encoding \gls{CReDiT} roles using Biber annotations:
...
@article{maguire2026pipeline,
  author    = {Maguire, Jr., Gerald Q. and Smith, Jane},
  author+an = {
    1:orcid="0000-0002-6066-7469", credit="Conceptualization, Software, Writing - original draft";
...
Wrapping the contribution section inside a \TeX\ group (\texttt{\begingroup \dots \endgroup}) ensures the modification automatically discards itself when the scope closes, leaving the main thesis bibliography unaffected:
