...
This document describes the third-cycle version of the thesis template I have developed for use at \gls{KTH}. This version was developed based on an examination of the thesis template \enquote{\foreignlanguage{swedish}{Avhandlingsmall Word}} from
...
\item There is no backside of the title page --- so the student cannot easily specify a copyright or choose to use one of the Creative Commons licenses\footnote{Interestingly, \gls{KI} requires that the thesis is published
electronically in \gls{KI} Open Archive under a Creative Commons license, for example CC-BY9. - see  §7.2.8 Public notification – "nailing" on page 23 of \textit{Rules for doctoral education at Karolinska Institutet}, \url{https://staff.ki.se/media/1742/download} and \textit{Policy for open publication at Karolinska Institutet} \url{https://staff.ki.se/research-support/policy-for-open-publication-at-karolinska-institutet}} nor specify the \gls{ISBN} and TRITA numbers.

\item The AW-template does not address the question of abstracts in languages other than English and Swedish. There is no reason to limit the thesis to having abstracts in only Swedish and English. As can be seen in \gls{DiVA} there are a variety of languages used for abstracts in addition to English and Swedish\footnote{In \gls{DiVA} there are abstracts in the following language codes: alb, ara, cat, chi, dan, dut, eng, fin, fre, ger, gre, heb, hin, hun, ice, ita, jpn, kur, lat, lav, nor, pol, por, rum, rus, scc, slo, spa, swa, swe, tur, ukr, und, and vie. Additionally, there are keywords in the following language codes: ara, cat, chi, dut, eng, fin, fre, ger, gre, hun, ice, ita, lat, lav, nor, per, pol, por, rum, rus, scc, slo, spa, swa, swe, tur, and ukr.}{}.

\item The choice of the font used for Heading 1 on the "Abstract" page is Figtree Medium 24 pt. Unfortunately, Figtree does not support Greek --- characters, so a student would not be able to have "{\NotoSansFont Περίληψη}" to indicate an abstract in the Greek language. Similar problems occur for other languages that use characters not included in Figtree. This same problem occurs when using Figtree in other heading levels and in the title and subtitle (if there were a cover or title page).
\end{itemize}
\clearpage
\begin{itemize}[resume*, label=\color{red}\ding{56}]
\item The abstract pages (and other preface pages) lack a page number, which would generally be a lowercase Roman numeral, \ie i, ii, iii, iv, etc.

\item There seems to be nothing done to make it easy to mechanically collect the abstracts for input to \gls{DiVA}, the National Subject Category information, the UN's \glspl{SDG} information, or other data that is needed for the \gls{DiVA} entry for input to \gls{DiVA}. This other data includes information about the author(s) and supervisors.

\item The acknowledgments page seems to be missing the so-called \enquote{acknowledgments signature} with the place, date, and author's name.
...
\item The \enquote{List of Publications} page in the AW-template seems to only consider peer-reviewed articles and scientific papers, and seems to ignore patents, patent applications, data sets, and other material that may have previously been published and be included as part of this thesis.

\item The \enquote{List of Publications} page says that the titles of the publications are to be set in Figtree; however, this may not be possible as Figtree lacks Greek characters, math symbols, \etc that may appear in the titles.

\item There is a similar problem with the Figtree font for the \enquote{List of
Abbreviations}.
...
\item The examples of tables show the use of numeric values set in Georgia Bold - which is \textbf{not} a fixed-width font, so numeric values will not nicely align vertically, making it more difficult for the reader to compare the values. Additionally, using the percentage symbol following each number when the row heading says \enquote{Percentage} is both redundant and confusing.
\end{itemize}
\clearpage
\begin{itemize}[resume*, label=\color{red}\ding{56}]
\item The footnote shown on the \enquote{Results} page shows the use of numeric footnotes -- this is generally a very poor choice in scientific and engineering theses as these numeric superscripts can be confused with exponents, such as $10^{2}$ or \qty{e4} and 1\ m\textsuperscript{3} or \qty{1}{\metre\squared}.  It is much better to use symbols (such as dagger, double dagger, etc.) and restart the symbols on each page.

\item The \enquote{Conclusions} chapter seems to be missing a subheading that addresses future work. It probably should also have a subheading of \enquote{Reflections on Ethical and Sustainability Issues} --- to be in keeping with the aim to identify these issues and clarify which were actually thought about. [In some cases, this is done earlier in the thesis --- but should be addressed somewhere.] \textbf{NB} \gls{DiVA} includes fields for listing which UN SDG goals a thesis contributes to.
...
Moreover, by not encouraging the use of \glspl{ISBN}, \glspl{DOI}, \glspl{URL}, etc. in the references (when they are relevant) makes the reader's job much harder when they want to access the actual reference. This also makes it much harder for tools to extract references and use them when reference counting.

\item  The example of a \enquote{Points of perspective} chapter seems very odd and would be very unusual in a technical thesis, but is found in the description of a compilation thesis at \gls{KI} (see \url{https://staff.ki.se/doctoral-education/time-to-defend-your-thesis/content-of-the-thesis}).
...
\clearpage
\begin{itemize}[resume*, label=\color{red}\ding{56}]
\item While the AW-template shows how one can include English and Swedish keywords, there is a need to support titles (and subtitles) in at least English and Swedish\footnote{In the case of my \LaTeX\ template, I have used the \enquote{subject} PDF document property to show the alternative title (for example, the Swedish title if the document's title is in English). In this way, when you open a document, you can quickly see in the document's properties the English and Swedish titles.}.
\end{itemize}
...
An additional unit is \enquote{half-points}. This unit is used to specify font sizes; therefore, a font size of 12pt equals 24 half points.

When specifying tables, one can use a relative measurement unit called \enquote{fiftieths of a Percent} (with the type \enquote{pct}).
...
\begin{enumerate}
    \item In the AW-template, Georgia Pro is used as the font for the default paragraph style and in the paragraph styles for \enquote{Publisher} and \enquote{EndnoteReference}.

    \item DM Sans - is indicated as the default document font and in the paragraph styles: \enquote{Kursiverat} and \enquote{Romerskaseparatlista};

    \item Tahoma - is used in the \enquote{Balloon Text} paragraph style
\end{enumerate}
...
caption={Part of the output about characters missing from Figtree that are actually used in \gls{DiVA} in English and Swedish titles and subtitles  - for the ITM school.},
...
\clearpage
\begin{lstlisting}[style=latexExampleForAuthors,  escapechar=|, 
caption={Part of the output about characters missing from Figtree that are actually used in \gls{DiVA} in English and Swedish titles and subtitles - for the SCI school.},
...
\end{Verbatim}
\FloatBarrier
Summarizing \Cref{lst:missingFigtreeSCI}, we see a number of Greek characters and ligatures, but we also see in diva2:628055 and diva2:628052 other problems -- specifically, a missing space and the Higgs decay channel expression being incorrectly entered in \gls{DiVA}. In \gls{HTML}, the first should be <em>H</em> \&rarr; <em>W W<sup>\&lowast;</sup></em> and the second should be <em>H</em> \&rarr; <em>W W<sup>\&lowast;</sup></em> \&rarr; <em>\&ell;\&nu;\&ell;\&nu;</em>.
\clearpage

\begin{lstlisting}[style=latexExampleForAuthors,  escapechar=|, 
caption={Part of the output about characters missing from Figtree that are actually used in \gls{DiVA} in English and Swedish titles and subtitles - for the CBH school.},
...
\begin{lstlisting}[style=latexExampleForAuthors,  escapechar=|, 
caption={Part of the output about characters missing from Figtree that are actually used in \gls{DiVA} in English and Swedish titles and subtitles - for the EECS school.},
...
\item[<Block Name>] Adds a human-readable description (e.g., \enquote{Greek and Coptic}, \enquote{Devanagari}), making it intuitive.
\item[.tex] Standard extension for LaTeX source files.
\end{description}

\Needspace*{5\baselineskip}
The content of these files in this template is based on those I found relevant or interesting when working on this template and its examples\footnote{I have tried to include most of the characters used in titles, subtitles, abstracts, and keywords of theses in \gls{DiVA} on or before 2025-05-28. As of that date, 2001 different characters had been used from a variety of fonts. After excluding characters that are in the Private Use Area (U+E000 - U+F8FF), and the Supplementary Private Use Area-B (U+100000 - U+10FFFF); along with non-printing control characters in Latin-1 Supplement (U+0080 - U+00FF). There are 1941 characters used.}. The file \texttt{lib/defines.tex} inputs the different files, so you are free to comment out the input if it is not relevant to your thesis or to add additional files as needed for your thesis.
...
The optional Dedication page does not have a heading. All of the other preface pages (except for the \enquote{Table of Contents}) have an unnumbered heading. The AW-template calls this paragraph style \enquote{\texttt{RomanList}}. However, I believe that this paragraph style \textbf{should} be called \enquote{\texttt{Unnumbered heading 1}}. This style sets the text in Figtree Medium 24pt.
Chapters should use this same style, but with numbering. This chapter numbering style is called \enquote{\texttt{Heading 1}} in the AW-template.

The table of contents heading uses a style called \enquote{\texttt{TOCHeading}} in the AW-template. This uses Figtree Medium 22pt and all caps. It is not clear to me why there is this difference in style. My assumption is that since the table of contents is automatically generated, this heading is also automatically generated and that this uses a style called \enquote{\texttt{TOC Heading}}. I think that to be more consistent with the other preface pages, this should also be set in Figtree Medium 24pt and \textbf{not} set in all caps.
...
\Needspace*{4\baselineskip}
At present, I do not understand why the capitalization and spacing are as they appear to be, with the paragraph style appearing in the list of paragraph styles as \enquote{Heading 1} - but it has the name and ID as shown above. I will try to write the names as they appear in the list of styles unless another ID or name is more suitable.
...
The first line on the cover shows the type of document and the subject, such as \enquote{Doctoral Thesis in Information and Communication Technology}. This text is set in Figtree Regular 10pt.
...
Of the three 2024 licentiate theses in Swedish, two refer to the type of thesis as \enquote{\foreignlanguage{swedish}{Licentitatavhandling}} and one as \enquote{\foreignlanguage{swedish}{Licentiatuppsats}}. And two use the connective as \enquote{\foreignlanguage{swedish}{INOM}} and one uses \enquote{\foreignlanguage{swedish}{i}}.

\Needspace*{14\baselineskip}
The licentiate \enquote{\foreignlanguage{swedish}{Koordinatbestämda fastighetsgränser: Analys av förutsättningar och förslag till reform av detsvenska gränssystemet}} by Linda Aldén - \url{https://kth.diva-portal.org/smash/record.jsf?pid=diva2%3A1891543} shows the name of the university under the author's name in English and the type and subject of the thesis  as \enquote{\foreignlanguage{swedish}{Licentitatavhandling inom fastigheter och byggande}}. This thesis uses the new cover format.
As does the licentiate thesis by Yael Bratel, \enquote{\foreignlanguage{swedish}{Urban rättvisa: Bortom segregation och ojämlikhet – stadsutformning för en rättvis stad}} and it states \enquote{\foreignlanguage{swedish}{Licentiatavhandling i Planering och Beslutsanalys}} - \url{https://kth.diva-portal.org/smash/record.jsf?pid=diva2%3A1907707}.
The licentiate thesis by Anna Strohmayer, \enquote{\foreignlanguage{swedish}{Vardagslivsrummet och meningsskapandet: Vardagsresande och social hållbarhet i den regionala planeringen i Barkarbystaden}} says that it is a \enquote{\foreignlanguage{swedish}{Licentiatuppsats i planering och beslutsanalys}} - \url{https://kth.diva-portal.org/smash/record.jsf?pid=diva2%3A1908492}. 

The doctoral thesis by Anna Hrdlicka, \enquote{\foreignlanguage{swedish}{Tidig medborgardialog i svensk detaljplanering: Kan den motiveras?}} 
-- \url{https://kth.diva-portal.org/smash/record.jsf?pid=diva2%3A1853301} - shows the type of thesis and subject as \enquote{\foreignlanguage{swedish}{Doktorsavhandling inom Fastigheter och byggande}}. Notee that this thesis uses an old style cover.
...
While trying to make a version of the cover, title page, and book info (i.e., copyright page), I noticed that in a number of the covers (One such thesis is the licentiate \enquote{\foreignlanguage{swedish}{Koordinatbestämda fastighetsgränser: Analys av förutsättningar och förslag till reform av det svenska gränssystemet}} by Linda Aldén - \url{https://kth.diva-portal.org/smash/record.jsf?pid=diva2%3A1891543} shows the name of the university under the author's name in English.)  that US-AB has set as \enquote{kth royal institute of technology} in Figtree Medium SC700 (Figtree-Medium-SC700) on the cover at 6.3pt. [The font size is based on the size used in the PostScript when printing on A4 paper and when viewed using Adobe Acrobat. Note that the title on the cover is in 24pt, the author 12pt, and the institution 6pt - as measured with my font ruler.]
...
I am unsure exactly where the glyphs in the above font originate from or how they were derived. US-AB has said that they produced them using (Adobe) Indesign. The embedded font's copyright string says \enquote{Copyright 2022 The Figtree Project Authors (https://github.com/erikdkennedy/figtree)}. Another possible means of generating them is using FontForge as described at \url{https://fontforge.org/docs/ui/dialogs/Styles.html}.
...
\begin{enumerate}
    ...
    \item As far as I know, Figtree does not have a small caps font, as such. Where did you get this font? Why are you using the lowercase string and small caps rather than setting the text as \enquote{KTH ROYAL INSTITUTE OF TECHNOLOGY} in Figtree Medium at 6.3pt?
...
Potentially, an author could implement a \enquote{Bottom-Bleed Layout} with the following characteristics:
\begin{itemize}
...
By adding the extra lines in the middle, I've basically kept the format - while providing a way to add the extra metadata (needed for \gls{DiVA}) in a visible form.
...
One of the important features of the template is the ability to have abstracts and keywords in a number of different languages - while at the same time making it easier to extract the abstracts and keywords for input to \gls{DiVA}.
...
Note that some subject areas commonly use structured abstracts, see the section \enquote{Structured abstracts} of \texttt{README\_3rd\_cycle\_author.tex}.
...
The AW-template says that the heading \enquote{Keywords} in the abstract should be set in Georgia Bold 12pt. Personally, I find this choice by the AW-template a bit ugly as one is using a serif font in what is functionally a heading -- when all of the other headings are in sans serif. Additionally, as noted earlier, Georgia Bold is a proprietary font.
...
As an abstract in \gls{DiVA} can contain \gls{HTML} code, it is technically possible to make tables and include figures in a \gls{DiVA} abstract. The problems are \first that I do not know a general means of transforming a table from LaTeX to \gls{HTML}, and \Second I do not know of a good way of passing the image (or images) that might be in a LaTeX abstract (via tools) to \gls{DiVA}. The second problem might be addressed by attaching the image files to the PDF and adding code to the generated \gls{HTML} to refer to these images. One problem with this is that the images would probably need to be uploaded as supplemental/supporting material, and then there would need to be some tool to update the abstract to refer to this uploaded file.

In a recent correction that I suggested for a thesis abstract in \gls{DiVA}, I manually created the table in \gls{HTML} to match the table that was in the abstract in the printed thesis.
\warningExpl{The \gls{DiVA} administrator who entered the thesis into \gls{DiVA} had not put \emph{any} of the abstract in, but simply put "See file" in the abstract field!}
...
\textbf{NB} Note that the words \enquote{Figures} and \enquote{Tables} are capitalized. Saying \enquote{List of Listings} would seem redundant; hence, I have simply titled it \enquote{Listings}.

The AW-template, seems to ignore the fact that it is often very useful to number figures, tables, listings with a hierarchical label \textit{within} the relevant section. For example, having a \enquote{Figure 2.2}, a \enquote{Table D.1}, and Listing \enquote{E.4}. Additionally, there is no need to name the entry in the list of figures, as \enquote{Figure 2.2}, it can simply be \enquote{2.2} with its description - as \enquote{Figure} is common to all of the entries in the list of figures.
...
\Needspace*{4\baselineskip}
It is also useful to know that in \LaTeX{} it is possible to specify a \enquote{short} description that is to be used in the List of \enquote{X} - just as one can do for headings that appear in the table of contents. The use of the optional short form enables a short description in the table of contents or List of \enquote{X}, while having a (very) long heading or description.
...
An optional section that is useful in some theses is a \enquote{List of Symbols Used}, \ie nomenclature. This list should indicate where a given symbol is defined. To have such a table included, one simply enables this option in the \texttt{\textbackslash documentclass}.
...
Stylistically, having such a list as a separate page does not make a lot of sense for a \textit{reader of the thesis}, as the list of publications and the author's contributions are generally given in the body of the thesis. If the purpose of this list is to facilitate \textit{a \gls{DiVA} administrator or the student} entering a list of the \textit{included publications} in \gls{DiVA}, then the list should list the publications in their full reference format, as even though the \gls{DiVA} page for the thesis will simply list them by title -- each of these titles has a hyperlink to a \gls{DiVA} page for the publication. Without having the full reference in this list, what is a suitable means of helping the reader find the full reference to the item being described? This template proposes a means of having a list of publications while collecting the relevant information about the included papers as part of the process of producing the divider pages.
...
\texttt{ListOfReports}, \texttt{ListOfPatentApplications},  \texttt{ListOfArtifacts}, and \texttt{ListOfDatasets}. You can create groups of publications as shown in the \enquote{List of Publications}\iflabelexists{chap:listOfPublications}{on page\,\pageref{chap:listOfPublications}}.
...
\Needspace*{5\baselineskip}
\subsection{Setting the List of References}

The template sets the title of the reference page to \enquote{References} using:
\begin{lstlisting}[style=latexExampleForAuthors]
...
\subsection{Extended Persistent Identifiers and Authority Resolvers}
\label{sec:cycle3ExtendedBiblatex}
Standard BibLaTeX models are often insufficient for modern engineering and cross-disciplinary dissertations:
\begin{itemize}
    \item Standard BibLaTeX provides native fields only for \texttt{doi}, \texttt{eprint}, \texttt{isbn}, and \texttt{issn}.

    \item Academic literature (as observed in KTH third-cycle theses) frequently relies on domain-specific, national, or repository-level persistent identifiers that standard styles either ignore or mangle (e.g., LIBRIS XL slugs, MathSciNet MR numbers, PubMed Central IDs, Archival Resource Keys, OSTI IDs, and Handle URIs).
\end{itemize}

To support these additional \glspl{PID}, the BibLaTeX data model architecture has been extended in this template via two modular files:
\begin{description}
    \item[Data Model Extension (\texttt{lib/ext-eprint-dbx.tex})] Emits \texttt{ext-eprint.dbx} to declare custom \texttt{verbatim} fields for exact string preservation without \TeX\ character-escaping conflicts.
    \item[Resolver and Formatting Pipeline (\texttt{lib/biblatex-and-friends.tex})] Provides driver patches, sourcemaps, and active hyperlinks (\texttt{\textbackslash ifhyperref}) wrapped in monospaced formatting (\texttt{\textbackslash nolinkurl}) pointing to global persistent resolvers (such as \url{https://n2t.net}, \url{https://libris.kb.se}, and \url{https://nbn-resolving.org}).
\end{description}

We categorize bibliographic identifiers into four architectural tiers:
\begin{description}
    \item[Tier 1: Native \texttt{biblatex} Fields] Pre-declared in standard BibLaTeX/Biber models (do \emph{not} add to \texttt{.dbx}):
    \begin{itemize}
        \item \texttt{doi} (covers Crossref, DataCite, Japan Link Center (JaLC))
        \item \texttt{eprint}, \texttt{eprinttype}, \texttt{eprintclass} (generic eprint handler)
        \item \texttt{isbn}, \texttt{issn}, \texttt{isrn}
        \item \texttt{url}, \texttt{urldate}
    \end{itemize}

    \item[Tier 2: High-Value Global PIDs] Identifiers with universal prefix syntax, stable institutional resolvers, or dedicated authority indexes (declared with \texttt{datatype=verbatim} in \texttt{ext-eprint.dbx} and summarized in \Cref{tab:addedGlobalPIDs}).

    The entries in \Cref{tab:addedGlobalPIDs} show the aliases defined for \texttt{mrnumber} and \texttt{pmid}]. These sourcemaps are provided to support authors whose reference managers (such as Zotero, Mendeley, or direct MathSciNet \BibTeX\ exports) still emit legacy field names.

\begin{table}[!ht]
  \centering
  \small
  \caption{Extended persistent identifier fields and their formatted bibliography output}
  \label{tab:addedGlobalPIDs}
  \begin{tabular}{p{2.8cm} | >{\raggedright\arraybackslash}p{6.8cm} | >{\raggedright\arraybackslash}p{5.2cm}}
    \textbf{Field} & \textbf{Authority / Target Registry} & \textbf{Rendered In-Bibliography Output} \\
    \hline
    \multicolumn{3}{l}{\textbf{\textit{Scholarly Repositories \& Domain Indexes}}} \\
    \texttt{arxiv}      & arXiv Open Access E-print repository     & arXiv:\ \nolinkurl{2301.12345} \\
    \texttt{mr}\newline\footnotesize(\texttt{mrnumber})        & AMS Mathematical Reviews (MathSciNet)    & MR:\ \nolinkurl{3456789} \\
    \texttt{zbl}        & FIZ Karlsruhe zbMATH Open                & Zbl:\ \nolinkurl{1234.56789} \\
    \texttt{pmcid}      & NCBI PubMed Central Full-Text ID         & PMCID:\ \nolinkurl{PMC22399602} \\
    \texttt{pubmed}\newline\footnotesize(\texttt{pmid}) & NCBI NLM PubMed (PMID)                   & PMID:\ \nolinkurl{22399602} \\
    \texttt{hdl}        & CNRI Handle System (RePEc, DSpace)       & HDL:\ \nolinkurl{10419/193838} \\
    \texttt{jstor}      & JSTOR Digital Archive                    & JSTOR:\ \nolinkurl{2134567} \\
    \texttt{osti}       & US Dept.\ of Energy (OSTI) Resource ID   & OSTI ID:\ \nolinkurl{20161947} \\
    \texttt{eric}       & US Dept.\ of Education (ERIC) ID         & ERIC:\ \nolinkurl{ED091983} \\
    \texttt{ssrn}       & Social Science Research Network ID       & SSRN:\ \nolinkurl{1505251} \\
    \texttt{adsbibcode} & NASA Astrophysics Data System (ADS)      & ADS bibcode:\ \nolinkurl{2024ApJ...123..456A} \\
    \texttt{oai}        & Open Archives Initiative Identifier      & OAI Identifier:\ \nolinkurl{oai:arXiv.org:2301.12345} \\
    \hline
    \multicolumn{3}{l}{\textbf{\textit{National Authority \& Union Library Catalogs}}} \\
    \texttt{libris}     & National Library of Sweden (LIBRIS XL)   & LIBRIS:\ \nolinkurl{7qj6drwk42flzpz} \\
    \texttt{urn}        & Uniform Resource Name (NBN Resolver)     & URN:\ \nolinkurl{urn:nbn:se:kth:diva-361272} \\
    \texttt{ark}        & Archival Resource Key (BnF, UNESCO)      & ARK:\ \nolinkurl{12148/bpt6k12345} \\
    \texttt{lccn}       & Library of Congress Control Number       & LCCN:\ \nolinkurl{49004389} \\
    \texttt{oclc}       & OCLC / WorldCat Accession Number         & OCLC:\ \nolinkurl{123456789} \\
    \texttt{gnd}        & DNB Integrated Authority File (Entity)   & GND:\ \nolinkurl{112441315X} \\
    \texttt{dnb}        & German National Library (Manifestation)  & DNB:\ \nolinkurl{956895318} \\
    \texttt{sudoc}      & French Higher Education Union Catalog    & SUDOC:\ \nolinkurl{044342294} \\
    \texttt{sbn}        & Italian National Library Service (BID)   & SBN:\ \nolinkurl{IT\textbackslash ICCU\textbackslash CUB\textbackslash 0198064} \\
    \hline
    \multicolumn{3}{l}{\textbf{\textit{Commercial \& Unlinked Indexing Identifiers}}} \\
    \texttt{asin}       & Amazon Standard Identification Number    & ASIN:\ \nolinkurl{B0007J1DLO} \\
    googlebooks & Google Books Record ID & Google Books: \nolinkurl{8OcHAAAAQAAJ} \\
    \texttt{scopus}     & Scopus Electronic Identifier (EID)       & Scopus EID:\ \nolinkurl{2-s2.0-85087095386} \\
    \nolinkurl{wos}        & Web of Science Accession Number (ISI)    & WoS:\ \nolinkurl{000257173900007} \\
    \hline
  \end{tabular}
\end{table}

    \item[Tier 3: Document Series \& Formal Specifications] Formal standards, agency reports, and legislative acts belong in standard entry types (\texttt{@report}, \texttt{@standard}, \texttt{@legislation}) using native fields:
    \begin{itemize}
        \item \textbf{Standards \& Industry Codes} (ISO, IEC, IEEE Std, CEN, AFNOR, DIN, ASTM, 3GPP, ETSI, CIGRE):\\
        $\rightarrow$ Use \texttt{@standard} or \texttt{@report} with \texttt{number = \{...\}} and \texttt{organization = \{\{...\}\}}.
        
        \item \textbf{Agency Technical Reports} (NASA STI/NTRS, AGARD, AIAA, AFRL, JPL, LANL, LLNL, RAND, NCHRP, IAEA TECDOC, WHO TRS, USGS):\\
        $\rightarrow$ Use \texttt{@report} with \texttt{type = \{Technical Report\}}, \texttt{number = \{...\}}, and \texttt{institution = \{\{...\}\}}.
        
        \item \textbf{Swedish Official Publications} (SFS, SOU, Ds, Prop., Skr., Trafikverket TDOK/TRV, Statskontoret, Boverket):\\
        $\rightarrow$ Use \texttt{@legislation} or \texttt{@report} with \texttt{number = \{SFS 2026:123\}}.
        
        \item \textbf{United Nations \& EU Documents} (UN Doc Symbol, CELEX, ELI, Official Journal, EBA, EEA, JRC):\\
        $\rightarrow$ Use \texttt{number = \{UNEP/EA.5/Res.14\}}, \texttt{eprint = \{...\}}, and \texttt{eprinttype = \{celex\}}.
    \end{itemize}

    \item[Tier 4: Ephemeral Local Catalog Keys (Excluded)] Do \emph{not} create \texttt{.dbx} fields for internal library inventory keys, temporary session IDs, or migration-fragile shelfmarks lacking persistent global resolvers.
\end{description}
\FloatBarrier

\subsection{Explicit Placeholder References}
\label{sec:placeholderReferences}

By creating an explicit \texttt{placeholder} metadata field, that is considered during document compilation, one can ensure that draft reference stubs do not easily pass through continuous integration pipelines or supervisor review undetected. Now consider the following entry declared in the bibliography:

\begin{lstlisting}[language=BibTeX, caption={{A \BibLaTeX\ entry containing an active \texttt{placeholder} directive}}]
@article{placeholderExample1,
  author      = {Doe, John and Smith, Jane},
  title       = {A High-Performance Framework for Low-Latency Image Classification},
  journal     = {Journal of Advanced Vision Systems},
  year        = {2025},
  placeholder = {Replace with authoritative IEEE/CVF conference publication and DOI once published in proceedings},
}
\end{lstlisting}

When cited in the text using \texttt{\textbackslash cite\{placeholderExample1\}}, the custom hook executes before \texttt{\textbackslash printbibliography}, triggering a twofold defensive safeguard:
\begin{enumerate}
    \item \textbf{Compiler Warning:} A build-time diagnostic is issued to standard output and the build log:
    \begin{quote}
    \texttt{Package biblatex Warning: Placeholder reference 'placeholderExample1' detected: Replace with authoritative IEEE/CVF conference publication and DOI once published in proceedings.}
    \end{quote}
    
    \item \textbf{Visual Markup in Print:} Rather than rendering as an apparently legitimate citation, the entry is prefixed in the bibliography list with an explicit high-visibility warning:
    \begin{quote}
    \fullcite{placeholderExample1}
    \end{quote}
\end{enumerate}

An important practical use for such placeholders occurs when submitting manuscripts to venues with double-blind review policies. Authors frequently anonymize their own prior publications or ongoing work to preserve anonymity during review, but must remember to restore the full author list, formal publication metadata, and \glspl{PID} for the camera-ready version upon acceptance. Conversely, if a manuscript is not accepted, the placeholder explicitly reminds authors to revise the reference into a preprint, technical report, or repository artifact rather than leaving a cryptic redacted placeholder reference.
...
The AW-template provides no guidance about how to include publications or how the divider pages should be formatted. In this template, section \enquote{Including the PDF of a publication} of \texttt{README\_3rd\_cycle\_author.tex}. shows how to include PDF files of publications, while \Cref{sec:cycle3DividerPages} describes and illustrates how to make divider pages.
