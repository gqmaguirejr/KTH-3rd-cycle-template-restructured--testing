...
The following generates some example output when using Approach A for the CReDiT information in the \texttt{.bib} file:
\begingroup
...
Define the slug-to-text mappings with \texttt{\textbackslash csdef}, as shown in \Cref{lst:biberCreditRolesIterate}. Using etoolbox list iteration (\texttt{\textbackslash docsvlist}), you can step through each author and test each of the 14 standard CReDiT slugs systematically. Note that the code in \Cref{lst:biberCreditRolesIterate} should be placed in the preamble, except for \enquote{6. Hook into finentry} which only needs to be invoked (within a \texttt{\textbackslash begingroup} \dots \texttt{\textbackslash endgroup}) when you need it for listing the author contributions in your thesis.
