...
language=[LaTeX]{TeX},
    ...
    tabsize=2,
    columns=flexible,
    morekeywords={subtitle, alttitle, altsubtitle, hostcompany, courseCycle, courseCode, courseCredits, programcode, degreeName, subjectArea, educationSubjectcode, nationalsubjectcategories, todo, ifbiblatex, subsection}
}
...
According to SCB (\url{https://www.scb.se/contentassets/221c69e399f0424b852b21236952e300/sprakkoder.xlsx}), there are also \gls{ISO} 639-2 codes for Yiddish with regions (see \Cref{tab:SCByiddish}). For Romanian, the \gls{ISO} 639-2 B code is "rum" and the ISO 639-1 code is  "ro". However, SCB gives additional \gls{ISO} 639-2 codes (see \Cref{tab:SCBromani}). For Northern Sami (Samiska, (norra)), the \gls{ISO} 639-2 is "sme" and the ISO 639-1 code is "se", but is there a region code that should be used? (See \Cref{tab:SCBsami}) For Tornedalen Finnish (\foreignlanguage{swedish}{Meänkieli}), the ISO 639-2 code is "fit", but I am unable to determine what \gls{ISO} 639-1 code should be used (see \Cref{tab:languagesMissing}). Is it "fi" with some region code? If so, what region code? 
...
Currently, there are no hardcoded built-in keyword annotations reserved specifically for \gls{CReDiT} (Contributor Roles Taxonomy, ANSI/NISO Z39.104-2022~\cite{niso_z39_104_2022}). However, standard canonical identifiers using the \enquote{Simplified Name} form can be utilized directly as annotation keys, as outlined in \Cref{tab:CReDiT_annotation}. The values of these can be: \enquote{lead}, \enquote{equal} (=true), or \enquote{supporting}.
...
This setup allows automated Python/\gls{CI} validation pipelines to extract and cross-check contributor statements directly from \texttt{.bib} files without manual re-entry.
...
The following generates some example output when using Approach A for the \gls{CReDiT} information in the \texttt{.bib} file:
\begingroup
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbackslash thefield\{entrykey\}: Prints or retrieves the active \texttt{.bib} entry key (\eg maguire2026pipeline).
...
Whenever this macro is invoked during bibliography rendering, \texttt{\textbackslash thefield\{entrykey\}} dynamically evaluates to whatever reference is currently being processed.
...
\end{lstlisting}
\begin{description}
    \item[Use in Document] A single \texttt{.bib} file can populate both the \textbf{Included Papers} section and the \textbf{Other Publications} appendix by filtering on  \texttt{\textbackslash hasentryannotation\{included\}} inside \texttt{\textbackslash defbibfilter} or \texttt{\textbackslash defbibcheck}.
...
Because JabRef uses the \texttt{.bib} file directly as its internal database (without converting entries into a generic internal schema), it is the most compatible tool:
...
Because reference managers lag behind Biber’s advanced data-model features, \textbf{managing annotations outside the GUI reference manager is the only stable design pattern}. For a KTH doctoral workflow, managing \glspl{ORCID} and \gls{CReDiT} through \texttt{publications\_map.json} and external Python/\gls{CI} scripts remains far more robust than relying on a reference manager.

This means:
\begin{enumerate}
    \item \texttt{references.bib} as the standard citation repository (which Zotero or JabRef can export safely), and
    \item \texttt{publications\_map.json} (managed via the Python/Streamlit wizard and GitHub Actions) as the pipeline that programmatically decorates entries with `author+an` at build time, ensuring reference managers never accidentally wipe out contributor roles or \glspl{ORCID} during library re-exports.
\end{enumerate}
...
\begin{enumerate}
\item \texttt{references.bib}: The upstream, GUI-managed store (exported directly from Zotero, Mendeley, or JabRef) containing canonical bibliographic descriptors (\eg \texttt{author}, \texttt{title}, \texttt{year}, \texttt{doi}).

\item \texttt{references-annotations.bib}: A companion overlay containing sparse entries with matching citation keys that declare only the extended data annotations (\eg \texttt{author+an}, \texttt{title+an}, or \texttt{entry+an}).
\end{enumerate}

Existing utilities such as \texttt{bibtool}, \texttt{bibtex-tools}, or \texttt{bibmerge} are designed to detect duplicate citation keys and discard redundant entries. In contrast, an automated Python pre-compilation step (\eg integrated into a GitHub Action or local pre-commit hook) can perform an \emph{additive, field-level unification}. By parsing both files and merging fields that share a matching key into a unified intermediate file (\eg \texttt{references-merged.bib}), reference managers can safely overwrite \texttt{references.bib} during iterative library exports without clobbering curated author roles or \glspl{ORCID}.

Such an overlay or split-file architecture (\texttt{references.bib} alongside \texttt{references-annotations.bib}) cleanly separates concerns while providing persistent annotations. As a result, students never have to manually re-annotate files after updating their reference library.
...
Under the split-file overlay architecture (\texttt{references.bib} + \texttt{references-annotations.bib}), an additive Python pre-processor can attach granular semantic metadata across several distinct bibliographic dimensions without reference-manager corruption. Examples of these other dimensions include:
\begin{enumerate}
...
\end{lstlisting}
        ...
        \item[Application] Custom formatting drivers can switch between language-sensitive labels without maintaining multiple \texttt{.bib} variants.
        \end{description}
\end{enumerate}

Because reference managers routinely discard or corrupt non-standard fields, isolating these annotations inside \texttt{references-annotations.bib} and merging them via Python guarantees that extended provenance, venue, and licensing data remain permanently intact.
...
Rather than overloading \texttt{.bib} entries with document-assembly directives, the current workflows decouple bibliographic citation data from administrative pipeline state:
\begin{description}[style=nextline, labelwidth = {1cm}, leftmargin = !]
    \item[External Source of Truth (\texttt{publications\_map.json})]
In the current workflow, inclusion (\texttt{status: "included"}), label assignments (\texttt{paper:A}, \texttt{techreport:A}), tab order (\texttt{tab\_index}), physical reprint paths (\texttt{file\_path}), page slices (\texttt{pdf\_pages}), scaling factors, and reproduction copyright notices (\texttt{permission\_text}) live entirely inside \texttt{publications\_map.json}. This separates immutable bibliographic facts (which belong in \texttt{references.bib}) from the mutable, student-curated decisions about how physical \gls{PDF} reprints are laid out in a compilation thesis.


    \item[Two-Way Upstream Sync (\gls{DiVA} Discovery)]
The automated workflow pulls publication metadata directly from \gls{DiVA} via the student's KTHID through GitHub Actions. In a pure in-\TeX{} annotation workflow, the candidate must hand-annotate raw \texttt{.bib} files. In your pipeline, the GitHub Action generates \texttt{lib/publications\_generated.tex} and \texttt{lib/publications\_dividers\_generated.tex} dynamically from \gls{DiVA} and \texttt{publications\_map.json}, removing the manual editing step entirely.
...
Your handling of massive collaborations (like the BNL Multi-Particle Spectrometer example) via \texttt{specific\_contributors} demonstrates why the wizard approach (\texttt{CReDiT\_Matrix\_Wizard.py}) is cleaner than annotating \texttt{.bib} files. In large multi-author experiments, attempting to attach 14 \gls{CReDiT} part annotations to individual authors inside a standard BibTeX string quickly becomes unwieldy. Generating a standalone \texttt{lib/thesis\_contributions\_generated.tex} file keeps the bibliography clean while producing an explicit, readable contribution matrix for the defense committee.
\end{description}

Your pipeline positions the \texttt{.bib} file as a traditional citation store, delegating document assembly, inclusion filtering, DiVA harvesting, and \gls{PDF} insertion to external Python and \gls{CI} automation where it is most maintainable.
...
\Needspace*{12\baselineskip}
\begin{lstlisting}[language={BibTeX}, breaklines=true, basicstyle=\small\ttfamily,  escapechar=|, caption={{)}}, label=lst:biberMegaCollaborationMetadata]
@article{atlas2026higgs,
  author    = {Lindenbaum, S. J. and {MPS Collaboration}},
  author+an = {
    % Only annotate specific contributors, lead analysts, or the student:
    3:specific-contributor="Maguire Jr., Gerald Q.";
    3:software="lead";
    3:formal-analysis="lead";
    3:note="Summer student: PDP-10 assembly-to-FORTRAN conversion & track display"
  }
}
\end{lstlisting}
...
In your \texttt{references-annotations.bib} overlay:
...
\begin{enumerate}
    \item \texttt{publications\_map.json} remains the interactive store holding\linebreak[4] \texttt{specific\_contributors} and \texttt{contribution\_note}.
    
    \item When the wizard generates or updates \texttt{references-annotations.bib}, the Python script inspects \texttt{entry.get("specific\_contributors")}.
...
It should be possible to mine the \texttt{publications\_map.json} file generated by \texttt{CReDiT\_Matrix\_Wizard.py} to produce and initial \texttt{references-annotations.bib} file with the \gls{CReDiT} annotations for authors.
