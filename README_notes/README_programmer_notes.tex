...
For each processed entry, \fname{bib\_cleanup.py} queries external APIs:
\begin{enumerate}
    \item \textbf{\glspl{DOI}:} Verified through the Crossref REST API (\texttt{api.crossref.org}) using a polite \texttt{User-Agent} header. Crossref internal test prefixes (e.g., \texttt{10.18810}, \texttt{10.5555}) are recognized and handled appropriately. Fallback check via DataCite.
    \item \textbf{\glspl{ISBN}:} Verified via a three-tier fallback: Crossref $\rightarrow$ Google Books \gls{API} $\rightarrow$ Open Library.
...
For documents with \glspl{DOI}, the code supports fuzzy title matching via string-similarity checks (\eg Levenshtein distance or Jaro-Winkler) against the registered title.

When an \gls{API} hit succeeds, the script compares the record's published year against the entry's \texttt{year} field (prioritizing \texttt{published-print} over \texttt{issued} and \texttt{published-online}). Any discrepancy is flagged as an analysis warning.

The program checks for citations of retracted works (currently limited to works with a \gls{DOI}), using Crossref's Retraction Watch Data (\url{https://gitlab.com/crossref/retraction-watch-data}, based on a dataset from Retraction Watch). 
...
\begin{itemize}
    ...
    \item \textbf{Automated \gls{DOI} / \gls{ISBN} Backfilling:} If an entry contains a valid \gls{URL} or arXiv key but omits a canonical \gls{DOI}, the pipeline could query the Crossref \texttt{/works?query.title} endpoint to suggest or automatically inject the missing persistent identifier into \fname{referencesUsed.bib}. \todoinline{Perhaps this should only be a message to the author --- rather than being silently added to \fname{referencesUsed.bib}, so that the author could add it to \fname{references.bib}.}

    \item \textbf{URL Liveness and Decay Detection:} \glspl{URL} provided in \texttt{@online} or \texttt{@misc} entries that lack a \gls{DOI} or \gls{ISBN} are not actively polled via HTTP \texttt{HEAD} requests to verify liveness or catch bit rot (HTTP 404/410), nor are fallback requests routed through the Internet Archive Wayback Machine \gls{API}.
