...
\Cref{lst:luaExample1} shows an example of a Lua program that generates a JSON file named \fname{output.json}. The idea would be to utilize Lua data tables to store the metadata, then write it out as a complete JSON file.
...
Expanding on the proof of concept in \Cref{sec:avoidCleanup}, \Cref{lst:luaExample2} shows how Lua can be used to write the \fname{fordiva.json} file with abstracts and keywords.
David Kolf's \textit{JSON module for Lua 5.1 - 5.4} from the file \fname{dkjson.lua} handles the encoding of a Lua table into \gls{JSON}, including ordering of the top-level keys\footnote{Sorting is only done of the \textbf{top-level} keys. There is no facility for sorting the elements of the table.} and escaping of the \LaTeX{} commands. The JSON file is generated by invoking the \texttt{\textbackslash GenerateFordivaJSON} macro after filling in the table in the macro \texttt{\textbackslash divainfo}.

\begin{lstlisting}[language={[5.2]Lua}, caption={Lua code snippet to write fordiva.json file including abstracts and keywords}, label=lst:luaExample2] 
%
...
The \texttt{\textbackslash divainfo} command generates the \enquote{For DiVA} information pages by putting the metadata into a Lua table and then writing the whole table out as JSON. The \fname{fordiva.json} file is subsequently read back into a listing.
...
To facilitate making use of the \texttt{fordiva.json} file, this file is attached to the \gls{PDF} file along with the \fname{acronyms.tex} file. Once the \fname{fordiva.json} and \fname{acronyms.tex} files are attached to the \gls{PDF} file, you can later extract these\linebreak[4] attached files using various existing tools or my program: \linebreak[4]\texttt{extract\_embedded\_files\_from\_PDF.py}.
...
\subsection{Cleaning up JSON should be transforming JSON}
\label{sec:cleaaningUpJSON}
If you save the \fname{fordiva.json} and \fname{acronyms.tex} files locally\footnote{Either by extracting them from the PDF file or you can download the acronyms.tex file from your project and can download the \fname{fordiva.json} with the \enquote{Other logs and files} button shown at the bottom of the log output window.}, then you can clean up and transform the \fname{fordiva.json} file. After having done this, you can make a \gls{MODS} file with the following commands:
...
I believe that this code can handle a large fraction of all of the titles and subtitles that have previously been used at \gls{KTH}. 
...
We can produce all three outputs with the command shown in \Cref{lst:JSONtoCalendarThreeoutputs}. Note that the program was run with the \fname{event.json} file (a \gls{JSON} file containing \gls{HTML}) to produce a Canvas course announcement.
...
For testing purposes, I also created functionality in \fname{JSON\_to\_calendar.py} to insert a fixed event (this was my first test) and to take as input a \gls{MODS} file. The MODS file was created from a \gls{DiVA} feed of theses presented in 2020 through to the 25\textsuperscript{th} of April 2021. However, one limitation that I found is that, other than myself, few people have been entering the date and time for the oral presentation. Unfortunately, since I wanted to test making calendar announcements, I needed the date and time!

\Needspace*{2\baselineskip}
\textbf{NB}: I have assumed that each degree project presentation lasts one hour. This assumption was made since the \gls{KTH} Cortina calendar API needs both a starting and ending time. There is currently no support for specifying the duration of third-cycle presentations. However, looking at a calendar event for a doctoral defense on 2025-06-05, only the starting time appears in the event.
...
No investigation has been made of the accessibility of the \gls{KTH} cover nor of the contents of the \gls{PDF} file (\ie the thesis itself). The \gls{PDF} output by Overleaf appears to be \gls{PDF} version 1.5 (\ie thus it is accessible via Acrobat version 6 or later). The \fname{examplethesis.tex} file has code to change this version.
...
Some additional colors are defined and used for \texttt{todonotes} – they should, of course, be removed before the thesis is finalized (as of course all of these issues should have been resolved!). The default background color for todonotes is orange (which is predefined as \#FF7F00). The other colors are shown in \Cref{lst:todonotesColors}. The \gls{KTH} colors are defined in \fname{kthcolors.tex}.
...
The template itself is written in \LaTeX{}~using UTF-8 characters. The template uses packages from \textit{TeX Live version 2024}\footnote{It can also be used with \textit{TeX Live version 2025}.}. It must be compiled with \LuaLaTeX{}  (which natively supports UTF-8 input).

The template is designed with a set of options to generate a thesis in English or Swedish\footnote{The option names for both languages must be given in title case, \ie with initial capital letters.}, as explained at the top of the \fname{examplethesis.tex} file (see \Cref{lst:document}).
\Needspace*{8\baselineskip}
...
The text in the template itself (\ie the \fname{examplethesis.tex} file) is in Swedish and English. The notes regarding the \LaTeX{} class file, the various \fname{lib} files, and the various \fname{README} files are currently only in English.

The default bibliographic style when using \BibTeX{} is my own adaptation of the IEEE Transactions format (\ie numbered citations, numbered references, references in order of use) with the extension of adding DOIs, URLs, and ISBNs (when relevant).

\subsection{Improving accessibility}
\label{sec:improvingAccessibility}
One method for improving accessibility would be to include the accessibility package, \ie \textbackslash usepackage\{accessibility\} as this would include a comment in the \gls{PDF} file for each equation with the \LaTeX{} that generated the equation. However, this package is no longer maintained and is incompatible with many other packages. For an introduction to what the \LaTeX{}-project has been working on, see \enquote{LATEX Tagged PDF -- A blueprint for a large Project}~\cite{MittelbachRowley2020}. Based upon this article and the status of the \texttt{tagpdf} package, I conclude that it is too early to worry about properly tagging \gls{PDF} files; thus, doing so will have to await the release of a package designed for production use.
...
The files and folders in Overleaf can be see by clicking the \enquote{File tree} icon. Some students add folders per chapter and reduce the main document to a skeleton that includes the other parts of the document.

There are currently instructions concerning the template in the \fname{README\_notes/README\_notes\_reduced.tex} file\footnote{You will need to compile it separately.}. 
...
Most students will only need to add acronyms to the \fname{acronyms.tex} file and perhaps additional packages to \fname{includes.tex} (in some cases, these packages may need to be added to the \fname{includes-after-hyperref.tex} file if there is a conflict with \texttt{hyperref}).
...
The thesis uses a set of \LaTeX{} commands to collect information about the student and the supervisor(s) in the \fname{custom\_configuration.tex} file. However, how can a student know what their KTHID and other necessary information is?

There are two alternatives:
\begin{itemize}
    \item The main supervisor or someone can use the method described in section \enquote{Make it simple from the start} in the file \texttt{README\_supervisor\_notes}.
...
Note that the option \enquote{hyphenation={RaggedRight}} can be used with the configuration of the package to set the license information with a ragged right margin rather that as a filled and justified paragraph.
...
In an effort to add the \gls{PDF} metadata via the \texttt{hyperref} package, I decided to add the English and Swedish keywords part of the \gls{PDF} metadata. However, in order to do this, I had to know the keywords \textit{before} the \textbackslash begin\{document\} command in the \LaTeX{}~file. To do so, I added three new commands to the \texttt{kththesis.cls} file, as shown in \Cref{lst:keywords}. The commands are used in the \fname{examplethesis.tex} file to set up the keywords in both English and Swedish as well as include a new set of \LaTeX{} commands to store the \gls{PDF} metadata (as shown in \Cref{lst:storingkeywords}) using a file called \fname{lib/pdf\_related\_includes.tex} (shown in \Cref{lst:informationforPDFfile}). Later, the keywords that have been stored are inserted into the \LaTeX{}~after their respective language abstracts. The title page of the thesis and the PDF metadata are shown in \Cref{fig:pdfMetadata}. Note that both the English and Swedish versions of the keywords are included in the metadata. Finally, the keywords appear (as expected) in the \enquote{For DiVA} data at the end of the \gls{PDF} file.
...
In keeping with the Swedish standard (\foreignlanguage{swedish}{Svenska skrivregler [Språkrådet 1.6.3 och 12.11.5]}) a \enquote{\foreignlanguage{swedish}{mellanslag tankstreck}} with a space before and after, \ie \enquote{ – } is used to separate the title and subtitle when in Swedish, this leads to a condition based on whether the document is in Swedish or not in the computation of \texttt{mytitle} and \texttt{myalttitle}.
...
The \gls{XMP} data utilizes a number of different schema and namespaces. These namespaces are shown as \enquote{xmlns:xxxx}  in \Cref{lst:pdfinfoOutput}. One namespace is \texttt{xmlns:dc} and this refers to the scheme is \enquote{Dublin Core} \footnote{see Dublin Core Metadata Initiative \url{https://www.dublincore.org/}}. The information about this namespace can be found at \url{http://purl.org/dc/elements/1.1/}. Each of the schema and namespace are described in the next part of the \gls{XML}, see \Cref{lst:pdfinfoOutputPart2}.
...
\Cref{lst:pdfinfoOutputPart12} shows the \texttt{xmp}-prefix elements in the file. This namespace is supposed to be defined at \url{http://ns.adobe.com/xap/1.0/}; however, this URL is not longer valid but the Adobe XMP Basic namespace is described at url{https://developer.adobe.com/xmp/docs/XMPNamespaces/xmp/}. In this specific case, the file provides the \texttt{CreateDate}, \texttt{xmp:ModifyDate}, \texttt{xmp:MetadataDate}, and \texttt{xmp:CreatorTool}. For the last of these the value is shown as \enquote{LaTeX with hyperref} even though the \XeLaTeX{} engine had been used.
\begin{lstlisting}[style=myXML,
...
Additionally, the commands (from \fname{lib/defines.tex}) are supported in abstracts: \textbackslash ie, \textbackslash eg, \textbackslash etc, \textbackslash etal, \textbackslash first, \textbackslash second, \textbackslash third, \ldots . Now that all the acronyms are spelled out, there is no problem with them when making a calendar entry or \gls{MODS} file.

\subsection{URLs in abstracts}
\label{sec:URLSinAbstracts}
Currently, \glspl{URL} in abstracts are supported in Canvas course room announcements and the Canvas calendar, but \textit{not} in the Cortina calendar – where the \gls{URL} is simply shown as text.
...
Another example of this for \textbackslash fullcite\{doe2025collaborative\} using the code in \Cref{lst:biberHookExample1B} is:
\newbox\orcidbox
...
There are two primary paradigms for encoding \gls{CReDiT} roles using Biber annotations, as shown in \Cref{lst:biberCreditPatternA} and \Cref{lst:biberCreditPatternB}.
...
\begin{itemize}
    \item \textbf{Biber `.bib` Annotation Delimiters:} Item annotations must be delimited by semicolons (`;`), not commas (`,`), with the item index explicit on each part (\eg `1:conceptualization="lead"; 1:software="lead"`).
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbackslash thefield\{entrykey\}: Prints or retrieves the active \fname{.bib} entry key (\eg maguire2026pipeline).

        \item \textbackslash strfield\{entrykey\}: Returns the unexpanded string representation of the entry key.

        \item \textbackslash currententrykey: An internal macro holding the literal key of the item currently being processed.
    \end{itemize}
    
    \item Explicitly Setting an Arbitrary Entry as "Current"
...
More commonly, \BibLaTeX\ provides \textbackslash entrydata\{<entrykey>\}\{<code/macros>\}, which temporarily switches the current entry context to <entrykey> and executes your custom code within that entry's data scope:
...
Usage Example: Printing the Current Entry Key Alongside Metadata
\Needspace*{8\baselineskip}
...
\begin{description}[labelwidth =\widthof{\textbf{credit@<degree>@<author\_idx>}}, leftmargin = !]
    \item[creditlabel@<role>] Static translation table mapping raw slug keys to typeset strings.

    \item[credit@<degree>@<author\_idx>] Dynamic runtime data containers accumulating roles during macro evaluation.
\end{description}
...
\Needspace*{27\baselineskip}
    \item Thesis Inclusion \& Paper Categorization (Compilation Theses)
...
\end{description}
\Needspace*{20\baselineskip}
    \item Institutional Affiliation \& Degree Eligibility Mapping
...
Considering the usage \enquote{Thesis Inclusion \& Paper Categorization (Compilation Theses)} above, there is a practical problem: 
...
\begin{description}[labelwidth =\widthof{\textbf{Proprietary Flat Schemas}}, leftmargin = !]
    ...
    \item[UI Editing] It does not parse the sub-properties (`1:orcid="...";`) into GUI checkboxes, creator cards, or linked badges. The entire annotation block appears as an unformatted raw string in an auxiliary field text box.
    \item[Feature Status] Native support for Biber data annotations has been discussed in feature issues (\eg Issue \#11505), but it remains an unparsed text field.
\end{description}

    \item Zotero \& Mendeley (Destructive / Incompatible)

Both tools convert bibliographic entries into an internal relational model that assumes a flat `author` table (`first\_name`, `last\_name`, `role`):
\begin{description}[labelwidth =\widthof{\textbf{Proprietary Flat Schemas}}, leftmargin = !]
    \item[Data Loss on Import] If a `.bib` file decorated with `author+an = { 1:orcid="..."; }` is imported, the parser discards the annotation entirely because it does not map to any creator field.
    \item[Export Inability] Neither tool can reconstruct `<field>+an` upon export unless an external layer like Better BibTeX (BBT) parses custom dummy strings stashed in the record's `Extra` field.
\end{description}
...
\paragraph{Practical Recommendation}\mbox{}\par
\smallskip

Because reference managers lag behind Biber’s advanced data-model features, \textbf{managing annotations outside the GUI reference manager is the only stable design pattern}. For a \gls{KTH} doctoral workflow, managing \glspl{ORCID} and \gls{CReDiT} through \fname{publications\_map.json} and external Python/\gls{CI} scripts remains far more robust than relying on a reference manager.

This means:
\begin{enumerate}
    \item \fname{references.bib} as the standard citation repository (which Zotero or JabRef can export safely), and
    \item \fname{publications\_map.json} (managed via the Python/Streamlit wizard and GitHub Actions) as the pipeline that programmatically decorates entries with `author+an` at build time, ensuring reference managers never accidentally wipe out contributor roles or \glspl{ORCID} during library re-exports.
\end{enumerate}

\paragraph{A Forward-Looking Architecture: Split-File Overlay}\mbox{}\par
\smallskip
...
\begin{enumerate}
\item \fname{references.bib}: The upstream, GUI-managed store (exported directly from Zotero, Mendeley, or JabRef) containing canonical bibliographic descriptors (\eg \texttt{author}, \texttt{title}, \texttt{year}, \texttt{doi}).
...
In Biber and \BibLaTeX{}, data annotations are completely generic: \emph{any} standard or custom field can take the \enquote{+an} suffix at the item, field, or entry level.
...
\end{lstlisting}
        \item[Application] BibLaTeX macros can selectively render a green verification checkmark beside verified DOIs or trigger build-time compiler warnings for unverified or dead links.
    \end{description}
...
\end{lstlisting}
    ...
    \item Editorial Curation \& Retraction Watches (\texttt{entry+an})
        \begin{description}[labelwidth =\widthof{\textbf{Application}}, leftmargin = !]
        \item[Context] Long-lived literature repositories occasionally contain retracted papers, expressions of concern, or errata.
        \Needspace*{6\baselineskip}
        \item[Syntax]
...
To prevent Biber and \LaTeX{} from crashing on mega-collaborations while still providing granular \gls{CReDiT} attribution for a student, you can use \textbf{sparse target indexing}, (\ie you only need to add the relevant additional author details):
...
\section{GitHub Automation and Workflows}
\label{sec:GitHubAutomation}

By hosting this template on GitHub, Continuous Integration and Continuous Delivery/Deployment (CI/CD) pipelines (GitHub Actions) automate thesis metadata management, publication ingestion, and reference verification.

\subsection{Architectural Overview}
\label{sec:CIArchitecture}

The repository integrates several autonomous workflows designed to eliminate manual synchronization between external institutional registries, bibliography files, and \TeX{} macros:
\begin{description}[style=nextline, labelwidth=\widthof{\textbf{Automated Publication Discovery}}, leftmargin=!]
    \item[Configuration Wizard] Executes \fname{scripts/config\_wizard.py} to validate and configure user options in \fname{custom\_configuration.tex}.

    \item[Automated Publication Discovery] Harvests candidate publication metadata directly from \gls{DiVA} using the author's \texttt{kthid}.

    \item[Controlled Inclusion Mapping] Ingests \fname{publications\_map.json} to resolve whether discovered records map to included papers (\eg \texttt{paper:A}) or non-included artifacts (\eg \texttt{patent:B}).

    \item[\LaTeX{} Structural Generation] Generates publication divider pages and preface summaries dynamically, ensuring correct math-mode rendering and Unicode character escaping.

    \item[Bibliography Verification and Pruning] Triggers \fname{scripts/bib\_cleanup.py} to cross-check cited works against remote \gls{PID} registries and compile an export-ready subset.
\end{description}

\subsection{Bibliography Cleanup Pipeline (\fname{bib\_cleanup.py})}
\label{sec:bibCleanupDev}

The script \fname{scripts/bib\_cleanup.py} implements external bibliographic validation and metadata hygiene.

\subsubsection{Execution and Artifact Resolution}
The pipeline accepts an optional build artifact flag (\texttt{--artifact}) to restrict validation exclusively to cited sources:
\begin{itemize}
    \item \textbf{Artifact Mode:} The script searches for Biber control files (\fname{output.bcf}) or auxiliary logs (\fname{output.aux}). If found, it extracts citation keys matching \verb|\abx@aux@cite{<sec>}{<key>}| or standard \verb|\citation{<key>}| primitives.
    \item \textbf{Headless / Standalone Fallback:} If no build artifacts exist (common in headless \gls{CI} runs or when committing from web environments), the script gracefully degrades to inspecting all entries present in \fname{references.bib}.
    \item \textbf{Placeholder Bypassing:} Entries marked with \texttt{placeholder = \{true\}} (or another text string as the value) are ignored to allow template scaffolding without failing validation.
\end{itemize}

\subsubsection{PID Verification Hierarchy and Cross-Checking}
For each processed entry, \fname{bib\_cleanup.py} queries external \glspl{API} to establish cryptographic and authoritative resolution:
\begin{enumerate}
    \item \textbf{\glspl{DOI}:} Verified through the Crossref REST \gls{API} (\texttt{api.crossref.org}) using a polite \texttt{User-Agent} header. Crossref internal test prefixes (\eg\ \texttt{10.18810}, \texttt{10.5555}) are recognized and bypassed. Datasets, software releases, and Zenodo artifacts unresolved by Crossref fall back to the DataCite REST \gls{API}.
    
    \item \textbf{\glspl{ISBN}:} Verified via a multi-tier fallback: Crossref $\rightarrow$ Google Books \gls{API} $\rightarrow$ Open Library (with automatic ISBN-13 to ISBN-10 normalization) $\rightarrow$ Wikidata citation index.
    
    \item \textbf{Patents:} Standard patent identifiers (\eg\ \texttt{US\ldots}) generate direct canonical target requests against Google Patents.
    
    \item \textbf{Direct \glspl{URL} and Liveness:} Entries lacking persistent identifiers (\eg\ \texttt{@online}, \texttt{@misc}, \texttt{@report}) are polled via HTTP \texttt{HEAD} requests (falling back to streamed \texttt{GET} for servers restricting \texttt{HEAD}) to confirm availability. Endpoints returning HTTP 401/403 are treated as live but crawler-restricted. If an endpoint is dead (HTTP 4xx/5xx or connection failure), the script queries the Internet Archive Wayback Machine Availability \gls{API}; if an archived snapshot exists, a \enquote{\texttt{DECAYED URL (Archived in Wayback)}} notice is surfaced.

    \item \textbf{Direct arXiv / eprint Resolution:} While some arXiv preprints assign a standard \gls{DOI} (\eg via the \texttt{10.48550} prefix), many citations only supply an \texttt{eprint} field without an explicit \gls{DOI}. A dedicated lookup against the arXiv REST \gls{API} is used to validate preprint identifiers and check whether a formal journal version has since been published. The program produces suggestions for additions to the \fname{references.bib} file.
\end{enumerate}

\paragraph{Cross-Field Sanity and Heuristic Checks}
Once a record resolves, secondary heuristics validate metadata consistency:
\begin{itemize}
    \item \textbf{Fuzzy Title and Container Matching:} \gls{API}-returned work titles and container/proceedings titles are checked against the entry's \texttt{title} and \texttt{booktitle} using normalized token similarity to avoid false alerts on conference proceedings.
    
    \item \textbf{Extended Publication Date Resolution:} The pipeline extracts publication dates prioritizing \texttt{published-print} over \texttt{issued}, \texttt{published-online}, and \texttt{created} to properly handle online-first works and standards. Any year discrepancies against the entry's \texttt{year} or ISO \texttt{date} fields are reported.
    
    \item \textbf{Retraction and Errata Detection:} Direct Crossref relations (\texttt{is-updated-by}) and Crossmark metadata arrays are parsed to flag retracted literature and surfaced corrections.
    
    \item \textbf{Automated \gls{DOI} Discovery:} Entries lacking a \gls{DOI} or \gls{ISBN} are cross-referenced against Crossref's bibliographic search \gls{API}. If a high-confidence match ($>0.85$ similarity) is identified, the script emits an actionable suggestion for the author to add the canonical identifier to \fname{references.bib}.
\end{itemize}



\subsubsection{Caching Infrastructure}
To avoid exhausting \gls{API} rate limits on large bibliographies, lookup payloads are hashed and cached:
\begin{equation*}
    \text{hash} = \text{SHA256}(\text{JSON}(\text{entry} \setminus \mathtt{STRIP\_FIELDS}))
\end{equation*}
Cached results persist in \fname{.bib\_validator\_cache.json}. When entries remain unaltered, the GitHub Action bypasses network requests completely.

\subsubsection{Output Generation and Sanitation}
The pipeline generates \fname{referencesUsed.bib}, which exclusively contains cited entries and strips local or sensitive metadata keys (including \texttt{abstract}, \texttt{file}, \texttt{groups}, \texttt{keywords}, and \texttt{annotation}). The sanitized file could be attached to a thesis so others can easily analyze the cited works.

\subsubsection{Missing Functionality and Future Extensions}
\label{sec:bibCleanupMissing}

While the current pipeline addresses primary identifier lookup, year sanity checks, and retraction alerts, several extensions are needed to achieve more complete bibliographic hygiene:

\begin{itemize}
    \item \textbf{Fuzzy Author Matching:} The current validation only confirms that a DOI, ISBN, or patent identifier resolves; that the publication years align; and performs a string-similarity check against the registered title. However, a similar check is not made of the elements of the author list. This leaves room for \enquote{phantom} citations where a valid \gls{DOI} is accidentally paired with unrelated citation metadata

    \item \textbf{Extended Persistent Identifier Registries:}
    \begin{itemize}
        \item \textbf{PubMed / Europe PMC:} Biomedical and bioinformatics entries indexed via \gls{PMID} or \gls{PMCID} are currently unsupported unless an accompanying \gls{DOI} is present.
        \item \textbf{MathSciNet / zbMATH:} Mathematical reviews and standard MR numbers are not currently resolved against the American Mathematical Society's endpoints.
        \item \textbf{Handles and URN:NBN:} National thesis and institutional repository identifiers (such as SwePub/\gls{DiVA} URN:NBN handles) lack direct resolution fallback.
    \end{itemize}

    \item \textbf{Retraction Watch Database Integration for Non-DOI Works:} Retraction checks currently rely on Crossref relationships and Crossmark metadata. Integrating direct lookups against the full Retraction Watch tabular dataset would enable detection of retracted entries that lack a registered \gls{DOI} (\eg retracted book chapters, conference papers, or preprints identified by title/author tuples).

    \item \textbf{Author Disambiguation and ORCID Ingestion:} When validating entries, the script does not currently harvest or cross-reference authors' \glspl{ORCID}, nor does it check whether author names conform to the CReDiT contributor roles taxonomy.

    \item \textbf{Checking against known mashups/frankenreferences:} If a registry of known mashups/frankenreferences existed, it would be desirable to check against it. \todoinline{Perhaps a starting point for such a registry is those from my study of 2025 third-cycle theses.}\footnote{Unfortunately, \gls{Npof} does not maintain an open, machine-readable \gls{API} or structured database of cited references or detected frankenreferences. Their decisions are \emph{not} available in machine-readable form, but only published as \glspl{PDF} and summary press releases. Therefore, extracting these references would require manual curation or a text-extraction (or \gls{OCR}) scraping pipeline.}
\end{itemize}

\section{Future work and work in progress}
\label{sec:futurework}
There are several areas where the template, supporting programs, and documentation can be enhanced. The programs and notebooks can be found in \url{https://github.com/gqmaguirejr/KTH-3rd-cycle-template-restructured--testing} or 
\url{https://github.com/gqmaguirejr/KTH-3rd-cycle-template-restructured}.
...
One of these programs is \fname{extract\_data\_from\_ISP.py}. It extracts some information from a \gls{PDF} file of the student's \gls{ISP} and generates a \gls{JSON} file. This is in an early stage, but it shows what can be done to extract data from a \gls{PDF} file of an \gls{ISP}.

The program \fname{ISP\_JSON\_to\_configuration\_info.py} takes the \gls{JSON} produced by \fname{extract\_data\_from\_ISP.py} and produces much of the information for the \fname{custom\_configuration.tex} file (with respect to the information available at the time the writing of the thesis starts).

To get additional information for the author and supervisors, the program uses information from either a Canvas course room \textbf{or} requires Canvas administrative rights to be able to look up the user's KTHID using the Canvas \gls{API} and the user's email address. In addition, the program utilizes the \gls{KTH} Profile \gls{API} to obtain further information about each person's organization, given an email address (if they are affiliated with \gls{KTH}).

\warningExpl{This program requires that you have a \gls{KTH} API key.}

\subsection{Automating creation of list of publications}
\label{sec:automatingPublicationsList}

The Jupyter notebook
\fname{Compute\_publications\_list\_from\_DiVA-Notebook-20250907.ipynb} uses data from \fname{fordiva.json} and \fname{references.bib} files to look up the student's publications in \gls{DiVA} and generate the \LaTeX{} that might be used in a list of publications in a third-cycle thesis.

This could be used for the \enquote{List of Publications} for a compilation thesis or for a list of papers and contributions in a monograph thesis. Note that the author will need to edit the resulting list to indicate which publications are included in a compilation thesis and which papers have not been included. The author may also need to move items from one category to another.
...
\warningExpl{Implementing JSON\_to\_Cora\_DiVA remains as future work.}
 

\subsection{Better exploiting unicode}
\label{sec:betterExploitingUnicode}

The current design stores the \LaTeX{} commands in abstracts \textit{without} expanding the \LaTeX{} macros. This makes it possible to post-process these commands and make content that is suited for making announcements of the oral presentation, making calendar entries for this presentation, and generating the abstracts for the \gls{MODS} file for \gls{DiVA}. However,  if one wants to take the full leap to unicode, this is perhaps not a good design choice, as it requires that all of the processing that \LaTeX{} could do of the unicode is not done and that the programs that extract the unexpanded contents will have to keep changing to keep up with what \LaTeX{} macros are used. 
...
This suggests a third alternative: have all of the contents (for titles, subtitles, keywords, and abstracts) be directly in unicode (or as characters specified as described in section \enquote{Configure the title, subtitle, alternate title, and alternate subtitle} in the file \fname{README\_3rd\_cycle\_author.tex}). See also \Cref{sec:generatingPlainText}. Perhaps some limited use of some macros can be allowed, as long as they can easily be processed into \gls{HTML} with unicode contents.
...
There is an open question of the extent to which the National Subject Category information, the UN's \glspl{SDG} list, and English and Swedish keywords can be automatically extracted or guessed (perhaps with the help of a large language model).
