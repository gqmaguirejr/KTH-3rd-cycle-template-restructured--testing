...
Some additional colors are defined and used for \texttt{todonotes} – they should, of course, be removed before the thesis is finalized (as of course all of these issues should have been resolved!). The default background color for todonotes is orange (which is predefined as \#FF7F00). The other colors are shown in \Cref{lst:todonotesColors}. The KTH colors are defined in \fname{kthcolors.tex}.
...
The thesis makes use of a set of \LaTeX{} commands to collect information about the student and the supervisor(s) in the \fname{custom\_configuration.tex} file. However, how can a student know what their KTHID and other necessary information is?
...
\subsection{Using Biber data annotations in BibLaTeX entries}
...
\Needspace*{18\baselineskip}
\begin{lstlisting}[language={BibTeX}, breaklines=true, basicstyle=\small\ttfamily, caption={{Example of Biber author annotations for ORCID and equal contribution}}, label=lst:biberAuthorAnnotation]
...
Note that each line of the annotation has a prefix of the form `<number>:' followed by a name, equal sign (`='), and a quoted value. Internally, this information is encoded into \textit{parts} and stored by Biber into the \texttt{`.bbl'} file. Note also the use of the \texttt{placeholder} field to explicitly identify this as a fake reference to be used simply for the purposes of examples.

\subsubsection{BibLaTeX Part Annotation Signature}
...
To render these annotations in print, one can define \BibLaTeX\ formatting hooks (as shown in \Cref{lst:biberHookExample1A}). You can test for active annotations using \texttt{\textbackslash haspartannotation} and retrieve values with \texttt{\textbackslash getpartannotation}. In this code, equal authorship is indicated with a superscript asterisk, while the name gets an  \texttt{\textbackslash orcidlink} hyperlink, \ie beside the name is the official green, circular \gls{ORCID} icon as an actionable, live hyperlink targeting the author's \gls{ORCID} record page. The icon itself is drawn as a resolution-independent vector path using TikZ/PDF literals rather than a raster image.
...
\subsubsection{Method A for inserting ORCID links}
...
\subsubsection{Method B for inserting ORCID links}
...
\subsubsection{Using annotation to store Contributor Roles Taxonomy (CReDiT) data}
...
\subsubsection{Architectural Takeaways for Your Pipeline}
\begin{itemize}
    ...
    \item \textbf{Kebab-Case Slugs:} Adopting the standard \gls{CReDiT} simplified names (\eg `writing-original-draft`, `data-curation`) avoids unescaped underscore collisions in TeX math mode and simplifies downstream JSON serialization.
...
\subsubsection{Listing roles grouped by degree}
...
For co-authored papers, evaluating committees often need to confirm whether an author was affiliated with KTH or performed the research under an external industrial partnership:
...
Because reference managers lag behind Biber’s advanced data-model features, \textbf{managing annotations outside the GUI reference manager is the only stable design pattern}. For a KTH doctoral workflow, managing \glspl{ORCID} and \gls{CReDiT} through \fname{publications\_map.json} and external Python/\gls{CI} scripts remains far more robust than relying on a reference manager.
...
\subsubsection{Contrasting annotations with current workflows}
...
\subsubsection{Support for Mega-Collaborations}
...
\subsubsection{Integrating annotations with current workflows}
...
In your Streamlit wizard and GitHub Actions workflow:
\begin{enumerate}
...
It should be possible to mine the \fname{publications\_map.json} file generated by \fname{CReDiT\_Matrix\_Wizard.py} to produce and initial \fname{references-annotations.bib} file with the \gls{CReDiT} annotations for authors.
